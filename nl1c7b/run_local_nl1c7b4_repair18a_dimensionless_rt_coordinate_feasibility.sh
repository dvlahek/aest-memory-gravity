...
R18_FREEZE_COMMIT='ff3517dc9a5d394339d20e82b3dd4158346f5498'
PREREG_COMMIT='7472d7be86c4de2b9f8f74463bad557757d0b60c'
IMPLEMENTATION_COMMIT='de0eba957933ba96ddb879c16928e5a288b7ed52'
LOCK_COMMIT='f29f83d04b17954d40c0b831109f6f7a24509861'

R18_FREEZE_BLOB='aa934847951036cfe925becd91740b35b3c0d065'
PREREG_BLOB='0b24906bc85d8f824053965d631be35de75d6af0'
IMPLEMENTATION_BLOB='767199e8ab620f5d6dabd50d0efde9828f22048b'
LOCK_BLOB='a03b3a1337c984649404f9aed5da637d7f48b6db'

R18_BLOB='c0e589fe120a148148c2e21406219e197d2c16db'
...
OUT='results/nl1c7b4_repair18a_dimensionless_rt_coordinate_feasibility.json'
LOG='results/nl1c7b4_repair18a_dimensionless_rt_coordinate_feasibility.log'
...
for c in "$R18_FREEZE_COMMIT" "$PREREG_COMMIT" "$IMPLEMENTATION_COMMIT" "$LOCK_COMMIT"; do
...
check_blob docs/nl1c7b4_repair18_result_freeze.md "$R18_FREEZE_BLOB"
check_blob docs/nl1c7b4_repair18a_predata_dimensionless_rt_coordinate.md "$PREREG_BLOB"
check_blob nl1c7b/initial_constraint_certification_repair18a.py "$IMPLEMENTATION_BLOB"
check_blob docs/nl1c7b4_repair18a_implementation_lock.md "$LOCK_BLOB"

check_blob nl1c7b/initial_constraint_certification_repair18.py "$R18_BLOB"
...
for f in "$R15A_JSON" "$R15A_NPZ" "$R16_JSON" "$R17_JSON" "$R18_JSON" "$TRACE" "$COVERAGE"; do
...
echo 'NL1C7B4_REPAIR18A_LOCK_PASS'
echo "HEAD=$(git rev-parse HEAD)"
echo "REPAIR18_JSON_SHA256=$R18_JSON_SHA"

mkdir -p results
set +e
PYTHONPATH="$PWD" python nl1c7b/initial_constraint_certification_repair18a.py   --trace "$TRACE"   --coverage-json "$COVERAGE"   --repair15a-json "$R15A_JSON"   --repair15a-npz "$R15A_NPZ"   --repair16-json "$R16_JSON"   --repair17-json "$R17_JSON"   --repair18-json "$R18_JSON"   --out "$OUT"   2>&1 | tee "$LOG"
rc=$?
set -e
...
p=pathlib.Path('results/nl1c7b4_repair18a_dimensionless_rt_coordinate_feasibility.json')
