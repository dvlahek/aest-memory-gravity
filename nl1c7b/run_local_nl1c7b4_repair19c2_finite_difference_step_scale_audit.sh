...
R19C1_FREEZE_COMMIT='cbf05b2a2845cda70fb962b20c5062d516b24a5b'
PREREG_COMMIT='a094d9b660b346c3fcf03f6b26f2ce39ce263894'
IMPLEMENTATION_COMMIT='515cc6c0ed04ec02d6dd48422bb0e96cf6575373'
LOCK_COMMIT='33190d51827fdfd84c5b4a7ed200f4ceac764045'

R19C1_FREEZE_BLOB='e61978b2d0a0af7f4ad4b2fbdeafc7b9626681fc'
PREREG_BLOB='7c2a0372beab748127353ef0124527b824158a0a'
IMPLEMENTATION_BLOB='802c37c03c75b9ca906b807dce3cbd1792a8a30f'
LOCK_BLOB='cbdacb14404f828304afa65cc4f284f8f07e571d'
...
OUT='results/nl1c7b4_repair19c2_finite_difference_step_scale_audit.json'
LOG='results/nl1c7b4_repair19c2_finite_difference_step_scale_audit.log'
...
for c in "$R19C1_FREEZE_COMMIT" "$PREREG_COMMIT" "$IMPLEMENTATION_COMMIT" "$LOCK_COMMIT"; do
...
check_blob docs/nl1c7b4_repair19c1_result_freeze.md "$R19C1_FREEZE_BLOB"
check_blob docs/nl1c7b4_repair19c2_predata_finite_difference_step_scale_audit.md "$PREREG_BLOB"
check_blob nl1c7b/initial_constraint_certification_repair19c2.py "$IMPLEMENTATION_BLOB"
check_blob docs/nl1c7b4_repair19c2_implementation_lock.md "$LOCK_BLOB"
...
for f in "$TRACE" "$COVERAGE" "$R15A_JSON" "$R15A_NPZ" "$R16_JSON" "$R17_JSON" "$R18_JSON" "$R18A_JSON" "$R18B_JSON" "$R18B1_JSON" "$R18C_JSON" "$R18D_JSON" "$R18D1_JSON" "$R19_JSON" "$R19A_JSON" "$R19B_JSON" "$R19B1_JSON" "$R19C_JSON" "$R19C1_JSON"; do
...
echo 'NL1C7B4_REPAIR19C2_LOCK_PASS'
echo "HEAD=$(git rev-parse HEAD)"
echo "REPAIR19C1_JSON_SHA256=$R19C1_JSON_SHA"

mkdir -p results
set +e
PYTHONPATH="$PWD" python nl1c7b/initial_constraint_certification_repair19c2.py   --trace "$TRACE"   --coverage-json "$COVERAGE"   --repair15a-json "$R15A_JSON"   --repair15a-npz "$R15A_NPZ"   --repair16-json "$R16_JSON"   --repair17-json "$R17_JSON"   --repair18-json "$R18_JSON"   --repair18a-json "$R18A_JSON"   --repair18b-json "$R18B_JSON"   --repair18b1-json "$R18B1_JSON"   --repair18c-json "$R18C_JSON"   --repair18d-json "$R18D_JSON"   --repair18d1-json "$R18D1_JSON"   --repair19-json "$R19_JSON"   --repair19a-json "$R19A_JSON"   --repair19b-json "$R19B_JSON"   --repair19b1-json "$R19B1_JSON"   --repair19c-json "$R19C_JSON"   --repair19c1-json "$R19C1_JSON"   --out "$OUT"   2>&1 | tee "$LOG"
rc=$?
set -e
...
p=pathlib.Path('results/nl1c7b4_repair19c2_finite_difference_step_scale_audit.json')
...
for r in d['case_rows']:
    print('scale=',r['scale_hinv_Mpc'],'Nr=',r['Nr'],
          '| ref_stability=',r['reference_stability_Dhalf_vs_D1'],
          '| default=',r['default_control'])
    for c in r['candidates']:
        print(' ',c['method'],'h=',c['abs_step'],
              '| mismatch=',c['directional_action_mismatch'],
              '| H=',c['H_action_mismatch'],
              '| M=',c['M_action_mismatch'],
              '| exact1=',c['one_step_probe']['frozen_residual_ratio'],
              '| rank=',c['one_step_probe']['rank'])
