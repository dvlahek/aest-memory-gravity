#!/usr/bin/env python3
"""GE19 Repair34 — canonical window-local reduced H4/Z21 particular construction.
...
def q_cross(mod6,mod7,bg,tag,z10,d10,z11,d11,nx,mu=1.0):
    """Balanced exact polarization of the homogeneous quadratic Q2 operator.

    This is algebraically identical to the Repair33 polarization but rescales
    both frozen directions to comparable Euclidean norm before subtraction,
    then restores the exact n10*n11 factor. No physical/source rescaling is
    introduced.
    """
    mm=float(mu)
    n10=float(np.sqrt(np.linalg.norm(z10)**2+np.linalg.norm(d10)**2))
    n11=float(np.sqrt(np.linalg.norm(z11)**2+np.linalg.norm(d11)**2))
    if not np.isfinite(n10) or not np.isfinite(n11) or n10<=0.0 or n11<=0.0:
        raise RuntimeError(f"nonfinite/zero Q-cross direction norm C={tag}: {n10}, {n11}")
...
def solve_config(mod6,mod7,bgs,sources,nt):
    ...
    return states,p0store,diag,controls
...
def main():
    ...
    statesP,p0P,solveP,ctlP=solve_config(mod6,mod7,bgs,srcP,NT_PRIMARY)
    statesT,p0T,solveT,ctlT=solve_config(mod6,mod7,bgs,srcT,NT_CONTROL)
    linear_max=max(ctlP["linear_residual_max"],ctlT["linear_residual_max"])
    state_time=state_time_control(bgs,statesP,statesT)
    shift=max(ctlP["shift_constraint_backward_error_max"],ctlT["shift_constraint_backward_error_max"])
    ...
    gates={
        ...
        "Q_cross_balanced_polarization_self_consistency_le_1e12":bool(qpolar<=Q_POLAR_MAX),
        ...
        "H4_shift_constraint_backward_error_le_1e6":bool(shift<=CONSTRAINT_MAX),
    ...
    report={
        "classification":(
            "GE19_REPAIR34_CANONICAL_WINDOW_LOCAL_REDUCED_H4_Z21_PARTICULAR_PASS"
            if passed else
            "GE19_REPAIR34_CANONICAL_WINDOW_LOCAL_REDUCED_H4_Z21_PARTICULAR_FAIL"
        ),
        "predata_classification":"GE19_REPAIR34_PREDATA_CANONICAL_WINDOW_LOCAL_REDUCED_H4_Z21_PARTICULAR",
        ...
        "source_controls":{
            "Q_cross_balanced_polarization_self_consistency_max":qpolar,
        ...
        "solve_controls":{
            ...
            "shift_constraint_backward_error_max":shift,
        ...
        "claim_boundary":"Repair34 certifies only the canonical window-local particular reduced H4/Z21 state on the frozen late-time scalar/AeST/pressureless-dust scope, using the same Repair18/Repair07 particular-state boundary/propagator convention as certified H3/Z20. It does not choose a primordial homogeneous Z21 mode, certify a full-species nonlinear cosmology, introduce finite eta, or establish an observational signal."
    }
...
if __name__=="__main__":
    try:
        ...
        print(json.dumps({
            "classification":"GE19_REPAIR34_IMPLEMENTATION_FAIL",
            "error":repr(exc),
