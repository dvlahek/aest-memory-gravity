...
echo "=== GE19 Repair02 lock audit ==="
test "$(git rev-parse HEAD:ge19/repair02_predata_frozen_symbol_tuple_capture.json)" = "286891c23de22bdf58a6d0a11684b5d43d857f9e"
test "$(git rev-parse HEAD:ge19/repair02_window_retarded_reduced_h3_z20_particular.py)" = "915b7883e30c4bd124154bee34c66ba5bc0f81cb"
test "$(git rev-parse HEAD:docs/ge19_repair02_frozen_symbol_tuple_implementation_lock.md)" = "7fc70a48bc2115b43ef6f81f3ad5fff14101fa8f"
test "$(git rev-parse HEAD:.github/workflows/ge19-repair02-prelock-audit.yml)" = "d1f89eb123f81c2d3a88d4f86a18ecc7968b9a69"
...
test "$(git rev-parse HEAD:docs/ge19_repair01_stable_exp_background_implementation_fail_freeze.md)" = "7cb06c432eee7b3b842582384e8e3c7266537184"
...
git merge-base --is-ancestor 6ed94ecefc5c2834012606ea092a84a8572f1d97 HEAD
git merge-base --is-ancestor 9a747d9347309712c83a0054299c6afd9d4b784b HEAD
git merge-base --is-ancestor b2fa89f5b934d55c527293a9f8068cff1c1f79e6 HEAD
git merge-base --is-ancestor 7e94427ada86c130e6ede0f948d3aa0b2267fdd5 HEAD

echo "GE19_REPAIR02_LOCK_PASS"

echo "=== GE19 Repair02 required local parents ==="
...
echo "GE19_REPAIR02_LOCAL_INPUTS_PRESENT"

python3 -m pip install numpy scipy sympy >/dev/null
python3 -m py_compile ge19/repair02_window_retarded_reduced_h3_z20_particular.py

rm -f   results/ge19_repair02_window_retarded_reduced_h3_z20_particular.json   results/ge19_repair02_window_retarded_reduced_h3_z20_particular.npz   results/ge19_repair02_window_retarded_reduced_h3_z20_particular_FULL.log

echo "=== GE19 Repair02 science execution ==="
set +e
python3 ge19/repair02_window_retarded_reduced_h3_z20_particular.py   --results-dir results   --json-out results/ge19_repair02_window_retarded_reduced_h3_z20_particular.json   --npz-out results/ge19_repair02_window_retarded_reduced_h3_z20_particular.npz   2>&1 | tee results/ge19_repair02_window_retarded_reduced_h3_z20_particular_FULL.log
RC=${PIPESTATUS[0]}
set -e

echo "=== GE19 Repair02 summary ==="
if test -s results/ge19_repair02_window_retarded_reduced_h3_z20_particular.json; then
python3 - <<'PY'
import hashlib, json
from pathlib import Path

p=Path("results/ge19_repair02_window_retarded_reduced_h3_z20_particular.json")
...
for fn in [
    "results/ge19_repair02_window_retarded_reduced_h3_z20_particular.json",
    "results/ge19_repair02_window_retarded_reduced_h3_z20_particular.npz",
    "results/ge19_repair02_window_retarded_reduced_h3_z20_particular_FULL.log",
]:
    q=Path(fn)
    if q.exists():
