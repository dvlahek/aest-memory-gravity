#!/usr/bin/env python3
"""GE19 Repair21 on-shell H1 parent + matched shift audit.

Diagnostic only. Re-solve the already-certified Repair13 reduced H1 system
on Nt=64 and Nt=128, rebuild the unchanged Repair14 H3 source on those
on-shell parents, apply the unchanged Repair18 projected boundary, and
evaluate propagated shift residuals on one common matched Nt128 grid.

Also reports the Repair20 separately-interpolated H1 parent as a
counterfactual. No physics, source formula, boundary rule, metric, threshold,
q20 or H4/Z21 edit.
"""
...
def main():
    ...
    for tag in r7.C_TAGS:
        interp_sources[tag]=r14.source_bundle_reduced_lambda(
    ...
    diag_gates={
        ...
        "boundary_Repair18_relative_L2_le_1e12":bool(boundary_repro<=BOUNDARY_REPRO_MAX),
    ...
    if not all(h1_gates.values()):
        ...
        route="INTERPOLATED_H1_PARENT_DEFECT_CONFIRMED"
    else:
        route="ACTIVE_SHIFT_ISSUE_PERSISTS_ON_ON_SHELL_PARENT"

    report={
        "classification":"GE19_REPAIR21_ON_SHELL_H1_PARENT_MATCHED_SHIFT_AUDIT_COMPLETE",
        "predata_classification":"GE19_REPAIR21_PREDATA_ON_SHELL_H1_PARENT_MATCHED_SHIFT_AUDIT",
        ...
        "H3_controls":{
            ...
            "Repair18_boundary_p0_reproduction_relative_L2_max":boundary_repro,
            "all_outputs_finite":h3_finite,
            "gates":diag_gates,
        },
        "routing":{"next_route":route},
        "interpretation_boundary":"Diagnostic only. Positive routing does not relabel Repair19 or certify Z20; a separately preregistered certification run is still required.",
...
if __name__=="__main__":
    try:
        ...
        print(json.dumps({
            "classification":"GE19_REPAIR21_ON_SHELL_H1_PARENT_MATCHED_SHIFT_AUDIT_IMPLEMENTATION_FAIL",
            "error":repr(exc),
        },indent=2))
        raise
