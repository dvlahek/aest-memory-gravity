#!/usr/bin/env python3
"""GE19 Repair07 window-retarded reduced-H3 Z20 particular solve.
...
def _csc_col_max_abs(A):
    A=A.tocsc()
    out=np.zeros(A.shape[1],float)
...
def _column_residuals(A,X,B):
    AX=A@X
    vals=[]
    for j in range(B.shape[1]):
        vals.append(float(
            np.linalg.norm(AX[:,j]-B[:,j])
            / max(np.linalg.norm(AX[:,j]),np.linalg.norm(B[:,j]),TINY)
        ))
    return np.asarray(vals,float)


def equilibrated_solve(A,B,max_refine=4):
    """Solve the same A x = B after deterministic diagonal equilibration.

    Dr and Dc are positive diagonal matrices:
        (Dr A Dc) y = Dr B,  x = Dc y.
    Residuals and refinement acceptance are always evaluated on the original
    unscaled system.
    """
    ...
    if one_rhs:
        BB=BB[:,None]
    if A.shape[0]!=A.shape[1] or BB.shape[0]!=A.shape[0]:
        raise RuntimeError("invalid equilibrated_solve dimensions")

    rowmax=_csr_row_max_abs(A)
    if np.any(~np.isfinite(rowmax)) or np.any(rowmax<=0.0):
        bad=np.where((~np.isfinite(rowmax)) | (rowmax<=0.0))[0].tolist()
        raise RuntimeError(f"zero/nonfinite matrix rows before equilibration: {bad[:20]}")
    rscale=1.0/rowmax
    Ar=A.multiply(rscale[:,None]).tocsr()

    colmax=_csc_col_max_abs(Ar)
    if np.any(~np.isfinite(colmax)) or np.any(colmax<=0.0):
        bad=np.where((~np.isfinite(colmax)) | (colmax<=0.0))[0].tolist()
        raise RuntimeError(f"zero/nonfinite matrix columns after row equilibration: {bad[:20]}")
    cscale=1.0/colmax

    As=Ar.multiply(cscale[None,:]).tocsc()
    Bs=rscale[:,None]*BB
    if not np.all(np.isfinite(As.data)) or not np.all(np.isfinite(Bs)):
        raise RuntimeError("nonfinite equilibrated linear system")

    try:
        lu=splu(As,permc_spec="COLAMD",options={"Equil":False})
        Y=lu.solve(Bs)
    except Exception as exc:
        raise RuntimeError(f"equilibrated sparse solve failed: {exc}") from exc
    X=cscale[:,None]*Y

    initial=_column_residuals(A,X,BB)
    current=float(np.max(initial))
    accepted=0
    for _ in range(int(max_refine)):
        R=BB-A@X
        try:
            DY=lu.solve(rscale[:,None]*R)
        except Exception as exc:
            raise RuntimeError(f"equilibrated refinement solve failed: {exc}") from exc
        cand=X+cscale[:,None]*DY
        cres=_column_residuals(A,cand,BB)
        cmax=float(np.max(cres))
        if not np.all(np.isfinite(cand)) or not np.all(np.isfinite(cres)):
            break
        if cmax < current:
            X=cand
            current=cmax
    ...
    final=_column_residuals(A,X,BB)
    diag={
        "row_max_dynamic_range":float(np.max(rowmax)/np.min(rowmax)),
        "row_scale_dynamic_range":float(np.max(rscale)/np.min(rscale)),
        "post_row_column_max_dynamic_range":float(np.max(colmax)/np.min(colmax)),
        "column_scale_dynamic_range":float(np.max(cscale)/np.min(cscale)),
        "scaled_matrix_abs_max":float(np.max(np.abs(As.data))) if As.nnz else 0.0,
        "initial_unscaled_relative_L2_max":float(np.max(initial)),
        "final_unscaled_relative_L2_max":float(np.max(final)),
        "accepted_refinement_steps":int(accepted),
        "all_solution_values_finite":bool(np.all(np.isfinite(X))),
    }
    if not diag["all_solution_values_finite"]:
        raise RuntimeError("nonfinite equilibrated solution")
    return (X[:,0] if one_rhs else X),diag
...
def _scalar_complex(v):
    ...
    if a.size!=1:
        raise RuntimeError(f"expected scalar local coefficient, got shape {a.shape}")
    return complex(a.reshape(-1)[0])


def _bg_point(bg,xq):
    x=np.asarray(bg["x"],float)
    def r(name):
        return float(PchipInterpolator(x,np.asarray(bg[name],float),extrapolate=False)(xq))
    return {
        "x":float(xq),
        "a":float(math.exp(float(xq))),
        "H":r("H"),
        "Q_action":r("Q_action"),
        "Z_action":r("Z_action"),
    }
...
def _raw_local_outputs(mod6,mod7,bg,tag,k,xq,w):
    ...
    vals6=(
        aa,adot,Zb,
        N,S,S,0.0,u,
        Sdot,ik*S,Sdot,ik*S,0.0,
        udot,ik*u,phidot,ik*phi,ik*N,
        KB,CV,K2,Q0,Z0,
    )
    p6={name:_scalar_complex(fn(*vals6)) for name,fn in mod6.f_c1.items()}
...
def _dense_equilibrated_solve(A,B):
    ...
    if np.any(row<=0.0) or np.any(~np.isfinite(row)):
        raise RuntimeError("zero/nonfinite row in canonical dense solve")
    Dr=1.0/row
    Ar=Dr[:,None]*A
    col=np.max(np.abs(Ar),axis=0)
    if np.any(col<=0.0) or np.any(~np.isfinite(col)):
        raise RuntimeError("zero/nonfinite column in canonical dense solve")
    Dc=1.0/col
    As=Ar*Dc[None,:]
    Bs=Dr[:,None]*B
...
def _source_interp(xgrid,source_by_beta,m):
    betas=list(BETAS)
    rr=np.stack([np.asarray(source_by_beta[b]["rhs"][:,:,m],complex) for b in betas],axis=2)
    cc=np.stack([np.asarray(source_by_beta[b]["rhs_constraint"][:,:,m],complex) for b in betas],axis=2)
    rr_re=PchipInterpolator(xgrid,rr.real,axis=1,extrapolate=False)
    rr_im=PchipInterpolator(xgrid,rr.imag,axis=1,extrapolate=False)
    cc_re=PchipInterpolator(xgrid,cc.real,axis=1,extrapolate=False)
    cc_im=PchipInterpolator(xgrid,cc.imag,axis=1,extrapolate=False)
    def rhs(xq):
        return np.asarray(rr_re(xq)+1j*rr_im(xq),complex)
    def con(xq):
        return np.asarray(cc_re(xq)+1j*cc_im(xq),complex)
    return rhs,con


def _zero_source_interp(ncol=1):
    def rhs(xq):
        return np.zeros((6,ncol),complex)
    def con(xq):
        return np.zeros((2,ncol),complex)
    return rhs,con
...
def _radau2_integrate_canonical(mod6,mod7,bg,tag,k,y0,rhsfun,conrhsfun):
    ...
    for i in range(len(x)-1):
        ...
        stages,diag=_dense_equilibrated_solve(K,RHS)
        Y1=stages[:8]; Y2=stages[8:]
        G1=M1@Y1+F1
        G2=M2@Y2+F2
...
def _reconstruct_canonical_solution(mod6,mod7,bg,tag,k,Y,rhsfun,conrhsfun):
    ...
    shift_piece=np.zeros(ncol,float)
    aniso_piece=np.zeros(ncol,float)
    ...
    for it,xq in enumerate(bg["x"]):
        ...
        for j in range(ncol):
            ...
            # Repair07: use the same cancellation-safe row-scaled
            # normalization already used for the independent lapse row.
    ...
    return state,dots,{
        ...
        "shift_constraint_piece_normalized_diagnostic_max":float(np.max(shift_piece)),
        "anisotropy_constraint_piece_normalized_diagnostic_max":float(np.max(aniso_piece)),
...
def solve_reduced_h1_case_canonical(mod6,mod7,bg,tag,reference,reference_dot):
    nt=len(bg["x"])
...
def solve_case_canonical(mod6,mod7,bg,tag,sources_by_beta):
    nt=len(bg["x"]); nb=len(BETAS)
    states=np.empty((nb,len(M_SOLVE),6,nt),complex)
...
def solve_reduced_h1_case(mod6,mod7,bg,tag,reference,reference_dot):
    ...
    for ik,m in enumerate(FOURIER_N):
        ...
        try:
            X,eqdiag=equilibrated_solve(A,B)
        except Exception as exc:
            raise RuntimeError(f"reduced H1 equilibrated solve failed C={tag} m={m}: {exc}") from exc
    ...
    return state,{
        ...
        "equilibrated_solve_diagnostics":equilibration,
        "equilibrated_solve_final_unscaled_relative_L2_max":float(max(
            (q["final_unscaled_relative_L2_max"] for q in equilibration),default=math.inf
        )),
    }
...
def reduced_state_real(bg,state,nx,dynamic_dot=None):
    """Reconstruct reduced Z10 and local derivatives in real space."""
    nt=len(bg["x"])
    Dx=fd4_matrix(nt,bg["x"][0],bg["x"][-1])
    Dt=bg["H"][:,None]*Dx
    ...
    for iv,name in enumerate(FIELDS):
        ...
        if dynamic_dot is not None and iv in (1,2,3,4):
            jd=(1,2,3,4).index(iv)
            dcoeff=np.asarray(dynamic_dot[:,jd,:],complex)
        else:
...
def source_real_reduced(mod6,mod7,bg,tag,beta,nx,reduced_state,reduced_dot=None):
    """H3 source evaluated on the internally reclosed reduced H1 state."""
    nt=len(bg["x"])
    Dx=fd4_matrix(nt,bg["x"][0],bg["x"][-1])
    Dt=bg["H"][:,None]*Dx
    real,dot,spatial=reduced_state_real(bg,reduced_state,nx,reduced_dot)
...
def source_bundle_reduced(mod6,mod7,bg,tag,nx,reduced_state,reduced_dot=None):
    out={}
    for beta in BETAS:
        out[beta]=rhs_fourier(
            source_real_reduced(mod6,mod7,bg,tag,beta,nx,reduced_state,reduced_dot)
        )
    return out
...
def main():
    ...
    for tag in C_TAGS:
        ...
        st64,dot64,ctl64=solve_reduced_h1_case_canonical(
            mod6,mod7,bg64,tag,ref64[tag],refdot64[tag]
        )
        st32,dot32,ctl32=solve_reduced_h1_case_canonical(
    ...
    provenance={
        ...
        "GE15_background_mode_mismatch_gate_status":"diagnostic_only",
        "active_linear_GE06_background_coordinate":"Z_action",
    ...
    if not (provenance_pass and stage_A_pass):
        result={
            "classification":"GE19_REPAIR07_WINDOW_RETARDED_REDUCED_H3_Z20_PARTICULAR_FAIL",
        ...
        "repair07_predata_amendment01":"GE19_REPAIR07_PREDATA_AMENDMENT01_COMPONENTWISE_BACKWARD_ERROR_SCALE",
            ...
            "claim_boundary":"Stage A failed or provenance failed, so amendment01 forbids construction or interpretation of Z20. Repair07 changes only cancellation-safe constraint-monitor normalization; no H3 threshold or model choice is changed."
        }
    ...
    for tag in C_TAGS:
        src1024[tag]=source_bundle_reduced(
            mod6,mod7,bg64,tag,NX_PRIMARY,h1_64[tag],h1_dot64[tag]
        )
        src2048[tag]=source_bundle_reduced(
            mod6,mod7,bg64,tag,NX_SPATIAL_CONTROL,h1_64[tag],h1_dot64[tag]
        )
        for beta in BETAS:
    ...
    solve_rows=[]
    for tag in C_TAGS:
        st,res,con,eqdiag=solve_case_canonical(mod6,mod7,bg64,tag,src1024[tag])
        state64_by_C[tag]=st
        ...
        solve_rows.append({
            ...
            "canonical_march_diagnostics":eqdiag,
        })
    ...
    for tag in C_TAGS:
        src32[tag]=source_bundle_reduced(
            mod6,mod7,bg32,tag,NX_PRIMARY,h1_32[tag],h1_dot32[tag]
        )
        st32,res32,con32,eqdiag32=solve_case_canonical(mod6,mod7,bg32,tag,src32[tag])
        ...
        time_rows.append({
            ...
            "control_canonical_march_diagnostics":eqdiag32,
        })
    ...
    classification=(
        "GE19_REPAIR07_WINDOW_RETARDED_REDUCED_H3_Z20_PARTICULAR_PASS"
        if passed else
...
if __name__=="__main__":
    try:
        ...
        # Keep Repair05 implementation failures distinct from science FAIL.
        print(json.dumps({
            "classification":"GE19_REPAIR07_WINDOW_RETARDED_REDUCED_H3_Z20_PARTICULAR_IMPLEMENTATION_FAIL",
            "error":repr(exc),
        },indent=2))
        raise
