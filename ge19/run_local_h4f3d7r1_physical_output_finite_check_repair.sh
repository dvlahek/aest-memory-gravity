#!/usr/bin/env bash
# GE19 H4F3d7r1 PHYSICAL original-R1 interval ODE versus sampled FD4 audit; single final finiteness implementation repair.
...
if [[ "$(git symbolic-ref --quiet --short HEAD)" != "physics-first-gravitational-elasticity" ]]; then
  echo GE19_H4F3D7R1_WRONG_BRANCH
  exit 4
fi
...
PINNED=(
  ...
  "ge19/h4f3d7r1_predata_physical_output_finite_check_repair.json:f404c1b910e9dbcb342fbc4d3403fb83108697f7"
...
for entry in "${PINNED[@]}"; do
  rel="${entry%%:*}"
  expected="${entry#*:}"
  [[ "$(git rev-parse "HEAD:$rel")" == "$expected" ]] || { echo "GE19_H4F3D7R1_CODE_BLOB_MISMATCH $rel"; exit 4; }
  git diff --quiet -- "$rel" || { echo "GE19_H4F3D7R1_DIRTY_CODE $rel"; exit 4; }
  git diff --cached --quiet -- "$rel" || { echo "GE19_H4F3D7R1_STAGED_CODE $rel"; exit 4; }
done
git diff --quiet -- ge19/run_local_h4f3d7r1_physical_output_finite_check_repair.sh
git diff --cached --quiet -- ge19/run_local_h4f3d7r1_physical_output_finite_check_repair.sh
echo "GE19_H4F3D7R1_PHYSICAL_LOCAL_LOCK_PASS"
echo "CODE_HEAD=$(git rev-parse HEAD)"
echo "RUNNER_BLOB=$(git rev-parse HEAD:ge19/run_local_h4f3d7r1_physical_output_finite_check_repair.sh)"
...
if [[ -z "${VIRTUAL_ENV:-}" ]]; then
  if [[ -f "$ROOT/.venv/bin/activate" ]]; then
    ...
    echo "GE19_H4F3D7R1_VENV_MISSING: source .venv/bin/activate"
    exit 5
  fi
fi
...
if [[ -z "$TRACE" || ! -s "$TRACE" ]]; then
  echo "GE19_H4F3D7R1_MISSING_OR_AMBIGUOUS_ORIGINAL_R1_TRACE: set GE19_REPAIR26_R1_TRACE to its original file"
  exit 6
fi
...
check_sha() {
  ...
  [[ -s "$path" ]] || { echo "GE19_H4F3D7R1_MISSING_PHYSICAL_FILE $path"; exit 6; }
  got="$(sha256sum "$path" | awk '{print $1}')"
  [[ "$got" == "$expected" ]] || { echo "GE19_H4F3D7R1_SHA256_MISMATCH $path $got"; exit 6; }
}
...
[[ "$(wc -c < "$TRACE" | tr -d ' ')" == "26643162" ]] || { echo GE19_H4F3D7R1_REPAIR26_R1_LENGTH_MISMATCH; exit 6; }
echo GE19_H4F3D7R1_FROZEN_H4F3B_H4F3D6_R1_SHA256_PASS

JSON="$ROOT/results/ge19_h4f3d7r1_physical_r1_fd4_vs_interval_ode.json"
NPZ="$ROOT/results/ge19_h4f3d7r1_physical_r1_fd4_vs_interval_ode.npz"
FULL="$ROOT/results/ge19_h4f3d7r1_physical_r1_fd4_vs_interval_ode_FULL.log"
for out in "$JSON" "$NPZ" "$FULL"; do
  [[ ! -e "$out" ]] || { echo "GE19_H4F3D7R1_OUTPUT_ALREADY_EXISTS_NO_OVERWRITE $out"; exit 7; }
done
...
print("GE19_H4F3D7R1_PHYSICAL_ORIGINAL_ALL_PARENT_INPUT_PASS")
print("CERTIFIED_Z11 =", files["z11"])
print("REPAIR13 =", files["r13"])
print("H3F =", files["h3fn"])
print("H3G =", files["h3gn"])
PY
echo GE19_H4F3D7R1_PHYSICAL_LOCAL_PREEXECUTION_PASS

set +e
python3 -B -m ge19.h4f3d7r1_physical_output_finite_check_repair --results-dir "$ROOT/results" --repair26-trace "$TRACE" --source-json "$SOURCE_JSON" --source-npz "$SOURCE_NPZ" --d6-json "$D6_JSON" --d6-npz "$D6_NPZ" --json-out "$JSON" --npz-out "$NPZ" 2>&1 | tee "$FULL" | tail -n 48
rc=${PIPESTATUS[0]}
set -e
...
if [[ "$rc" -eq 0 && -s "$JSON" && -s "$NPZ" ]]; then
  echo GE19_H4F3D7R1_PHYSICAL_DECOMPOSITION_PASS_FULL_NOETHER_OPEN
elif [[ "$rc" -eq 2 && -s "$JSON" && -s "$NPZ" ]]; then
  echo GE19_H4F3D7R1_PHYSICAL_DIAGNOSTIC_FAIL
else
  echo GE19_H4F3D7R1_PHYSICAL_IMPLEMENTATION_OR_INPUT_FAIL
fi
exit "$rc"
