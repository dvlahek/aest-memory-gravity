#!/usr/bin/env bash
# GE19 H4F3d11 exact-source original nonbath background E00 physical diagnostic.
# All eight homogeneous Euler rows, separate FD4/FD8. No on-shell or full H4 Ward claim.
set -euo pipefail
umask 077
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
[[ "$(git symbolic-ref --quiet --short HEAD)" == "physics-first-gravitational-elasticity" ]] || {
  echo GE19_H4F3D11_WRONG_BRANCH; exit 4;
}
PINS=(
 ...
 "ge19/run_local_h4f3d10r1_lossless_fourier_archive_repair.sh:4bc186f7f6185198fc53cc6e072a221214ed81ad"
 "ge19/h4f3d10r1_actual_physical_lossless_archive_independent_audit.json:2921f79dbb0ce99200418bfba924b699ff8cd67f"
 "ge19/h4f3d11_predata_actual_original_action_background_e00.json:deb4b35e6a147b48fe93d6c99d798572650000ac"
 "ge19/h4f3d11_source_bound_original_background_partials.py:1f085efec4ea64b42822ddeaac566f532d9dccf1"
 "ge19/h4f3d11_actual_original_action_background_e00.py:b162fad1db2a855917b5f8e29fce10c67d30737f"
...
for entry in "${PINS[@]}"; do
  ...
  [[ "$(git rev-parse "HEAD:$rel")" == "$want" ]] || { echo "GE19_H4F3D11_BLOB_MISMATCH $rel"; exit 4; }
  git diff --quiet -- "$rel" || { echo "GE19_H4F3D11_DIRTY $rel"; exit 4; }
  git diff --cached --quiet -- "$rel" || { echo "GE19_H4F3D11_STAGED $rel"; exit 4; }
done
git diff --quiet -- ge19/run_local_h4f3d11_actual_original_action_background_e00.sh
git diff --cached --quiet -- ge19/run_local_h4f3d11_actual_original_action_background_e00.sh
echo GE19_H4F3D11_ORIGINAL_CODE_LOCK_PASS

if [[ -z "${VIRTUAL_ENV:-}" ]]; then
  [[ -f "$ROOT/.venv/bin/activate" ]] || { echo GE19_H4F3D11_VENV_MISSING; exit 5; }
...
[[ -n "$TRACE" && -s "$TRACE" ]] || { echo GE19_H4F3D11_MISSING_OR_AMBIGUOUS_R1_TRACE; exit 6; }
TRACE="$(realpath "$TRACE")"
check() {
 local path="$1" want="$2" got
 [[ -s "$path" ]] || { echo "GE19_H4F3D11_MISSING $path"; exit 6; }
 got="$(sha256sum "$path" | awk '{print $1}')"
 [[ "$got" == "$want" ]] || { echo "GE19_H4F3D11_SHA_MISMATCH $path $got"; exit 6; }
}
...
echo GE19_H4F3D11_ORIGINAL_PHYSICAL_INPUT_SHA_PASS

JSON="$ROOT/results/ge19_h4f3d11_actual_original_action_background_e00.json"
NPZ="$ROOT/results/ge19_h4f3d11_actual_original_action_background_e00.npz"
FULL="$ROOT/results/ge19_h4f3d11_actual_original_action_background_e00_FULL.log"
for dest in "$JSON" "$NPZ" "$FULL"; do
  [[ ! -e "$dest" ]] || { echo "GE19_H4F3D11_NO_OVERWRITE $dest"; exit 7; }
done
TMP="$(mktemp -d)"
trap 'rm -rf -- "$TMP"' EXIT
mkdir -p "$TMP/results"
...
echo GE19_H4F3D11_ISOLATED_LOCAL_EXECUTION
set +e
python3 -B -m ge19.h4f3d11_actual_original_action_background_e00 --results-dir "$ROOT/results" --repair26-trace "$TRACE" --json-out "$JSON" --npz-out "$NPZ" 2>&1 | tee "$FULL" | tail -n 28
rc="${PIPESTATUS[0]}"
set -e
...
if [[ "$rc" -eq 0 && -s "$JSON" && -s "$NPZ" ]]; then
  echo GE19_H4F3D11_ACTUAL_BACKGROUND_E00_ARRAYS_DIAGNOSTIC_PASS_ONSHELL_OPEN
else
  echo GE19_H4F3D11_PHYSICAL_IMPLEMENTATION_OR_SOURCE_CONSISTENCY_FAIL
fi
exit "$rc"
