#!/usr/bin/env python3
"""GE19 Repair32B — factor-two corrected reduced H2/Z11 reconstruction.

Solve the corrected directional H2 equation

    L_GE06 Z11 = -2 M1_GE05[Z10,q10]

using the exact raw-residual normalization dictionary derived in Repair32A.
All Repair30 parents, grids, boundary rules and numerical solvers are retained.
The historical Repair30 local shift and max-per-field time monitors remain
report-only here. Repair32B validates the corrected reconstruction but does
not certify Z11 and does not license H4/Z21.
"""
...
RAW_RESIDUAL_DICTIONARY_SCALE=2.0
...
def source_from_B(bg,B):
    """Exact -2*M1_GE05 RHS mapped into the GE06 raw-residual convention.

    Repair32A derives GE05_M1_to_GE06_raw_residual_scale = 2 exactly.
    ...
    for im,m in enumerate(r7.FOURIER_N):
        k=float(m*r7.g9.K_REQ[0]/r7.FOURIER_N[0])
        rhs[im,2]=RAW_RESIDUAL_DICTIONARY_SCALE*0.5*bg["Q_action"]*bg["a"]**3*B[im]
        rhs[im,3]=RAW_RESIDUAL_DICTIONARY_SCALE*(-0.5)*bg["a"]**2*(1j*k)*B[im]
    return rhs
...
def state_time_control(xp,sp,xc,sc):
    ...
    return {
        "by_field_relative_L2":by,
        "max":float(mx),
        "full_six_field_global_relative_L2":rel_l2(si,sc),
        "dynamic_S_u_varphi_T_global_relative_L2":rel_l2(si[:,[1,2,3,4],:],sc[:,[1,2,3,4],:]),
        "delta_varrho_absolute_L2_difference":float(np.linalg.norm(si[:,5,:]-sc[:,5,:])),
        "delta_varrho_primary_interp_L2":float(np.linalg.norm(si[:,5,:])),
        "delta_varrho_control_L2":float(np.linalg.norm(sc[:,5,:])),
    }
...
def main():
    ...
    ap.add_argument("--repair32a-json",required=True)
    ap.add_argument("--json-out",required=True)
    ap.add_argument("--npz-out",required=True)
    args=ap.parse_args()

    rd=Path(args.results_dir)
    r32a_path=Path(args.repair32a_json)
    if not r32a_path.exists():
        raise RuntimeError("missing frozen Repair32A dictionary parent")
    if sha256(r32a_path)!=R32A_JSON_SHA:
        raise RuntimeError("Repair32A JSON hash mismatch")
    d32a=json.loads(r32a_path.read_text())
    if d32a.get("classification")!="GE19_REPAIR32A_GE06_GE05_RAW_RESIDUAL_NORMALIZATION_DICTIONARY_PASS":
        raise RuntimeError("Repair32A parent not PASS")
    ddict=d32a.get("derived_dictionary",{})
    if ddict.get("GE05_M1_to_GE06_raw_residual_scale")!=2 or ddict.get("fitted_normalization_used") is not False:
        raise RuntimeError("Repair32A dictionary factor is not the frozen exact factor 2")
    ...
    timeglobalmax=0.0; timedynmax=0.0
    ...
    for tag in r7.C_TAGS:
        ...
        timeglobalmax=max(timeglobalmax,tc["full_six_field_global_relative_L2"])
        timedynmax=max(timedynmax,tc["dynamic_S_u_varphi_T_global_relative_L2"])
    ...
    validation_gates={
        ...
        "Repair32A_dictionary_factor_exactly_2":bool(RAW_RESIDUAL_DICTIONARY_SCALE==2.0),
        ...
        "primary128_vs_control64_full_six_field_global_relative_L2_le_5e3":bool(timeglobalmax<=TIME_MAX),
        "primary128_vs_control64_dynamic_global_relative_L2_le_5e3":bool(timedynmax<=TIME_MAX),
    ...
    historical_Repair30_monitors_report_only={
        "shift_constraint_componentwise_backward_error_max":shiftmax,
        "anisotropy_constraint_componentwise_backward_error_max":anisomax,
        "max_per_field_primary128_vs_control64_state_relative_L2":timemax,
        "not_used_for_Repair32B_PASS":True,
    }
    passed=bool(all(validation_gates.values()))

    report={
        "classification":(
            "GE19_REPAIR32B_FACTOR2_CORRECTED_REDUCED_H2_Z11_RECONSTRUCTION_PASS"
            if passed else
            "GE19_REPAIR32B_FACTOR2_CORRECTED_REDUCED_H2_Z11_RECONSTRUCTION_FAIL"
        ),
        "predata_classification":"GE19_REPAIR32B_PREDATA_FACTOR2_CORRECTED_REDUCED_H2_Z11_RECONSTRUCTION",
        "uses_observational_data":False,
        "finite_physical_eta":False,
        "H4_Z21_solve_performed":False,
        "provenance":{
            "input_sha256":hashes,
            "Repair32A_JSON_sha256":sha256(r32a_path),
        ...
        "H2_equation":"L_GE06 Z11 = -2 M1_GE05[Z10,q10]",
        "raw_residual_dictionary":{
            "GE05_M1_to_GE06_raw_residual_scale":RAW_RESIDUAL_DICTIONARY_SCALE,
        ...
        "memory_source_control":{
            ...
            "GE05_to_GE06_raw_residual_scale":RAW_RESIDUAL_DICTIONARY_SCALE,
        },
        "solve_controls":{
            ...
            "primary128_vs_control64_state_relative_L2_max_report_only":timemax,
            "primary128_vs_control64_full_six_field_global_relative_L2_max":timeglobalmax,
            "primary128_vs_control64_dynamic_global_relative_L2_max":timedynmax,
            "per_case":solve_rows,
        ...
        "historical_Repair30_monitors_report_only":historical_Repair30_monitors_report_only,
        "Z11_constructed":True,
        "corrected_H2_reconstruction_validated":passed,
        "Z11_certified":False,
        "routing":{
            "next_route":(
                "PREREGISTER_REPAIR32C_ARTIFACT_ONLY_Z11_CERTIFICATION"
                if passed else
                "FACTOR2_CORRECTED_H2_RECONSTRUCTION_NOT_VALIDATED"
            ),
            "H4_Z21_licensed":False,
        },
        "claim_boundary":"Repair32B validates only the factor-two corrected reduced H2/Z11 reconstruction. Historical Repair30 local monitor failures are report-only. It does not certify Z11, solve H4/Z21, introduce finite eta, or make observational claims.",
    }
...
if __name__=="__main__":
    try:
        ...
        print(json.dumps({
            "classification":"GE19_REPAIR32B_IMPLEMENTATION_FAIL",
            "error":repr(exc),
            "Z11_certified":False,
            "H4_Z21_solve_performed":False,
        },indent=2))
