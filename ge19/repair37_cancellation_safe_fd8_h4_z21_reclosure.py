#!/usr/bin/env python3
"""GE19 Repair37 — cancellation-safe FD8 reduced H4/Z21 reclosure.

Construct

  L_total(GE06+GE07+Lambda) Z21 =
      -2 Q_total(Z10,Z11)
      -2 DY2[Z10;Z11]
      -2 M1_GE05[Z20,q20]
...
def load_parents(rd:Path,args):
    ...
    if missing:
        raise RuntimeError("missing Repair37 frozen inputs: "+", ".join(missing))
    expected={
...
def fd8_matrix(n:int,x0:float,x1:float)->np.ndarray:
    """Ninth-point, degree-8 exact first derivative on a uniform grid."""
    if n<9:
        raise ValueError("fd8 requires n>=9")
    h=(x1-x0)/(n-1)
...
def fd8_polynomial_exactness(n:int=17)->float:
    """Maximum absolute derivative error for monomials x^0..x^8."""
    x=np.linspace(-0.7,0.4,n)
    D=fd8_matrix(n,float(x[0]),float(x[-1]))
    err=0.0
    for p in range(9):
        y=x**p
        dy=np.zeros_like(x) if p==0 else p*x**(p-1)
        err=max(err,float(np.max(np.abs(D@y-dy))))
    return err
...
def q_cross_direct(qdirect,bg,tag,z10,d10,z11,d11,nx):
    ...
    Dx=fd8_matrix(len(bg["x"]),bg["x"][0],bg["x"][-1])
    Dt=bg["H"][:,None]*Dx
...
def lambda_cross_direct(bg,z10,d10,z11,d11,nx):
    """Exact Repair14 mixed Lambda coefficient with cancellation-safe audit."""
    ...
    # Cancellation-safe independent expanded exact bilinear formula.
    exact=np.zeros_like(main)
    exact[0]=(-36.0*rr*aa*S0)*S1
    exact[1]=(
        (-36.0*rr*aa*N0)*S1
        +(-36.0*rr*aa*N1)*S0
        +(-36.0*rr*S0)*S1
    )
    exacterr=rel_l2(main,exact)

    # Historical subtractive polarization is retained report-only.
    ...
    return main,con,exacterr,polerr,symerr
...
def assemble_nonmemory(qdirect,mod6,mod7,bg,tag,z10,d10,z11,d11,nx_dy):
    """Direct GE06/GE07/Lambda Q cross + unchanged DY2, with FD8 time assembly."""
    ...
    lam_m,lam_c,lam_exacterr,lam_polerr,lam_symerr=lambda_cross_direct(
    ...
    return (
        ...
        symmetry,cancellation_report,lam_exacterr,lam_polerr,lam_symerr
    )
...
def build_source_config(
    ...
    out={}
    qsym=[]; qcancel=[]; qlexact=[]; qlpol=[]; qlsym=[]
    for tag in r7.C_TAGS:
        bg=bgs[(nt,tag)]
        ...
        qga,qm,ql,dy,qsymerr,qcancelerr,qlexacterr,qlpolerr,qlsymerr=assemble_nonmemory(
            qdirect,mod6,mod7,bg,tag,z10,d10,z11,d11,nx_dy
        )
        qsym.append(qsymerr)
        qcancel.append(qcancelerr)
        qlexact.append(qlexacterr)
        ...
        out[tag]=bybeta
    return out,max(qsym,default=0.0),max(qcancel,default=0.0),max(qlexact,default=0.0),max(qlpol,default=0.0),max(qlsym,default=0.0)
...
def main():
    ...
    qlexact=max(qlexactP,qlexactS,qlexactQ,qlexactT)
    ...
    fd8_exact=fd8_polynomial_exactness()
    fd8_control={
        "stencil_points":9,
        "polynomial_exact_degree":8,
        "polynomial_test_abs_error_max":float(fd8_exact),
        "time_primary":NT_PRIMARY,
        "time_control":NT_CONTROL,
        "replaces_fd4_only_in_GE06_GE07_nonlinear_Euler_Lagrange_Dt_partial":True,
    }
    ...
    gates={
        ...
        "Q_lambda_direct_vs_exact_expanded_relative_L2_le_1e12":bool(qlexact<=Q_SYMMETRY_MAX),
        "Q_lambda_direct_symmetry_relative_L2_le_1e12":bool(qlsym<=Q_SYMMETRY_MAX),
        "FD8_degree8_polynomial_exactness_abs_le_1e10":bool(fd8_exact<=1e-10),
    ...
    report={
        "classification":(
            "GE19_REPAIR37_CANCELLATION_SAFE_FD8_H4_Z21_RECLOSURE_PASS"
            if passed else
            "GE19_REPAIR37_CANCELLATION_SAFE_FD8_H4_Z21_RECLOSURE_FAIL"
        ),
        "predata_classification":"GE19_REPAIR37_PREDATA_CANCELLATION_SAFE_FD8_H4_Z21_RECLOSURE",
        ...
        "source_controls":{
            ...
            "Q_lambda_direct_vs_exact_expanded_relative_L2_max":qlexact,
            "Q_lambda_subtractive_polarization_relative_L2_report_only":qlpol,
            "Q_lambda_direct_symmetry_relative_L2_max":qlsym,
            "FD8_time_derivative_control":fd8_control,
        ...
        "claim_boundary":"Repair37 certifies only the canonical window-local particular reduced H4/Z21 state on the frozen late-time scalar/AeST/pressureless-dust+Lambda scope. Relative to Repair36 it changes only two numerical diagnostics/discretizations: the cancellation-dominated Lambda polarization audit is replaced by an exact expanded bilinear audit, and the GE06/GE07 nonlinear Euler-Lagrange Dt@partial assembly uses a 9-point degree-8 exact time derivative on the same Nt128/Nt64 grids. The physical H4 equation, parents, Lambda terms, memory normalization, Repair18 boundary, Repair07 Radau propagator, and all science thresholds remain unchanged. It does not choose a primordial homogeneous Z21 mode, certify a full-species nonlinear cosmology, introduce finite eta, or establish an observational signal."
    }
...
if __name__=="__main__":
    try:
        ...
        print(json.dumps({
            "classification":"GE19_REPAIR37_IMPLEMENTATION_FAIL",
            "error":repr(exc),
