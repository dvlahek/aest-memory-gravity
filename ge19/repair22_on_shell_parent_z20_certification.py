#!/usr/bin/env python3
"""GE19 Repair22 on-shell-parent Z20 certification.

Certification run only. Recomputes the locked Repair21 on-shell-H1/H3
pipeline, adds the frozen Repair14 Nx1024/Nx2048 source convergence control
on the genuine Nt128 H1 parent, re-audits the Repair18 projected boundary,
and applies the preregistered active/near-null shift certification.

No physics/source/boundary/threshold edit. No q20 or H4/Z21.
"""
...
def main():
    ...
    r21=load_module(
        ROOT/"ge19/repair21_on_shell_h1_parent_matched_shift_audit.py",
        "ge19_repair21_for_repair22",
    )
    tmpj=Path(args.json_out).with_name(Path(args.json_out).stem+".repair21_tmp.json")
    tmpn=Path(args.npz_out).with_name(Path(args.npz_out).stem+".repair21_tmp.npz")
    old_argv=list(sys.argv)
    captured=io.StringIO()
    try:
        sys.argv=[
            str(ROOT/"ge19/repair21_on_shell_h1_parent_matched_shift_audit.py"),
            "--results-dir",str(rd),
            "--json-out",str(tmpj),
            "--npz-out",str(tmpn),
        ]
    ...
    if d21.get("routing",{}).get("next_route")!="INTERPOLATED_H1_PARENT_DEFECT_CONFIRMED":
        raise RuntimeError("recomputed Repair21 core did not reproduce positive route")
    ...
    for nt in (128,64):
        for tag in r7.C_TAGS:
            ...
            src[(nt,tag)]=r14.source_bundle_reduced_lambda(
    ...
    for tag in r7.C_TAGS:
        ...
        src2048[tag]=r14.source_bundle_reduced_lambda(
    ...
    boundary_rows=[]
    bp_repro=0.0; bscaled=0.0; blapse=0.0; bshift=0.0; balg=0.0
        ...
        for tag in r7.C_TAGS:
            for m in r7.M_SOLVE:
                ...
                for ib,beta in enumerate(r7.BETAS):
                    ...
                    pr=rel_l2(p,p18[(tag,float(beta),int(m))])
                    ...
                    boundary_rows.append({
                        ...
                        "projected_p0_relative_L2_vs_Repair18":pr,
    ...
    boundary_gates={
        "Repair18_projected_p0_relative_L2_le_1e12":bool(bp_repro<=P_REPRO_MAX),
    ...
    report={
        ...
        "predata_classification":"GE19_REPAIR22_PREDATA_ON_SHELL_PARENT_Z20_CERTIFICATION",
        ...
        "boundary_certification":{
            "projected_p0_relative_L2_vs_Repair18_max":bp_repro,
        ...
        "project_boundary":{
            ...
            "Repair21_interpolated_H1_parent_defect_confirmed":True,
            ...
            "q20_ready_after_freeze":passed,
            "Z21_licensed":False,
        },
        "claim_boundary":"PASS certifies only the low-mode constraint-certified window-local reduced-H3 Z20 particular directional state on the Repair13 self-consistent background with on-shell H1 parents. It does not certify a primordial/homogeneous second-order mode, full species, finite eta, physical-amplitude nonlinear evolution, lensing or observations.",
    }
...
if __name__=="__main__":
    try:
        ...
        print(json.dumps({
            "classification":"GE19_REPAIR22_ON_SHELL_PARENT_Z20_CERTIFICATION_IMPLEMENTATION_FAIL",
            "error":repr(exc),
        },indent=2))
        raise
