#!/usr/bin/env python3
"""GE19 Repair33 — window-local reduced H4/Z21 particular construction.
...
def build_context(rd,r13npz):
    ...
    mod6f=r7.load_frozen_generator(ROOT/"ge06/analytic_aest_directional_source_generator.py","ge06r33")
    mod6=r7.build_stable_ge06_generator_v2(mod6f)
    mod7=r7.load_frozen_generator(ROOT/"ge07/pressureless_matter_directional_source_generator.py","ge07r33")
    ge05=r7.load_frozen_generator(ROOT/"ge05/memory_directional_source_generator.py","ge05r33")
    normf,normdiag=r24.build_normalized_c2_generator(ge05)
    return bgs,bgctl,mod6,mod7,ge05,normf,normdiag

def q2_direction_parts(mod6,mod7,bg,tag,state,dot,nx):
    """GE06/GE07 raw Q2(d,d) on one reduced first-order direction."""
    nt=len(bg["x"])
...
def q_cross(mod6,mod7,bg,tag,z10,d10,z11,d11,nx,lam=1.0):
    lp=float(lam)
    pp=q2_direction_parts(mod6,mod7,bg,tag,z10+lp*z11,d10+lp*d11,nx)
    pm=q2_direction_parts(mod6,mod7,bg,tag,z10-lp*z11,d10-lp*d11,nx)
    return tuple((a-b)/(4.0*lp) for a,b in zip(pp,pm))
...
def assemble_nonmemory(mod6,mod7,bg,tag,z10,d10,z11,d11,nx_dy):
    """Q-cross at exact polynomial grid + DY2 at requested high resolution."""
    q1=q_cross(mod6,mod7,bg,tag,z10,d10,z11,d11,NX_POLY,1.0)
    qh=q_cross(mod6,mod7,bg,tag,z10,d10,z11,d11,NX_POLY,0.5)
    qerr=max(rel_l2(a,b) for a,b in zip(q1,qh))

    ga_m,ga_c,ma_m,ma_c=q1
    ...
    return (qga_main,qga_con),(qm_main,qm_con),dy_by_beta,qerr
...
def build_source_config(
    ...
    for tag in r7.C_TAGS:
        ...
        qga,qm,dy,qerr=assemble_nonmemory(mod6,mod7,bg,tag,z10,d10,z11,d11,nx_dy)
        qerrs.append(qerr)
        ...
        out[tag]=bybeta
    return out,max(qerrs,default=0.0)
...
def solve_config(mod6,mod7,bgs,sources,nt):
    states={}; diag={}; linmax=0.0
    for tag in r7.C_TAGS:
        st,lr,cr,eq=r7.solve_case(mod6,mod7,bgs[(nt,tag)],tag,sources[tag])
        states[tag]=st
        linmax=max(linmax,float(np.max(lr)))
        diag[tag]={
            "linear_residual_max":float(np.max(lr)),
            "historical_component_constraint_residual_max_report_only":float(np.max(cr)),
            "equilibration":eq,
        }
    return states,diag,linmax
...
def constraint_global(mod6,mod7,bgs,states,sources,nt):
    num=[0.0,0.0]; den=0.0
    for tag in r7.C_TAGS:
        bg=bgs[(nt,tag)]
        for ib,beta in enumerate(r7.BETAS):
            src=sources[tag][float(beta)]
            for jm,m in enumerate(r7.M_SOLVE):
                k=float(m*r7.g9.K_REQ[0]/r7.FOURIER_N[0])
                y=states[tag][ib,jm][:,:,None]
                main,con=r7.linear_operator_batch(mod6,mod7,bg,tag,k,y)
                lm=np.asarray(main[:,:,0],complex)
                lc=np.asarray(con[:,:,0],complex)
                rm=np.asarray(src["rhs"][:,:,m],complex)
                rc=np.asarray(src["rhs_constraint"][:,:,m],complex)
                den += max(np.linalg.norm(lm),np.linalg.norm(rm),TINY)**2
                for ic in (0,1):
                    num[ic] += np.linalg.norm(lc[ic]-rc[ic])**2
    scale=math.sqrt(den)
    return {
        "shift_global_relative_to_operator_scale":float(math.sqrt(num[0])/max(scale,TINY)),
        "anisotropy_global_relative_to_operator_scale":float(math.sqrt(num[1])/max(scale,TINY)),
        "global_main_operator_scale":float(scale),
    }

def zero_boundary_control(bgs,states,nt):
    mx=0.0
    for tag in r7.C_TAGS:
        bg=bgs[(nt,tag)]
        Dt=bg["H"][:,None]*r7.fd4_matrix(nt,bg["x"][0],bg["x"][-1])
        st=states[tag]
        for iv in r7.DYNAMIC_FIELD_INDEX:
            mx=max(mx,float(np.max(np.abs(st[:,:,iv,0]))))
            der=np.einsum("j,bmj->bm",Dt[0],st[:,:,iv,:],optimize=True)
            mx=max(mx,float(np.max(np.abs(der))))
    return float(mx)
...
def main():
    ...
    bgs,bgctl,mod6,mod7,ge05,normf,normdiag=build_context(rd,r13npz)
    ...
    srcP,qpolP=build_source_config(
        mod6,mod7,ge05,bgs,r22,r32b,pbath,b2048,NQ_PRIMARY,NT_PRIMARY,NX_DY_PRIMARY
    )
    srcS,qpolS=build_source_config(
        mod6,mod7,ge05,bgs,r22,r32b,pbath,b2048,NQ_PRIMARY,NT_PRIMARY,NX_DY_CONTROL
    )
    srcQ,qpolQ=build_source_config(
        mod6,mod7,ge05,bgs,r22,r32b,qbath,b1024,NQ_CONTROL,NT_PRIMARY,NX_DY_PRIMARY
    )
    srcT,qpolT=build_source_config(
        mod6,mod7,ge05,bgs,r22,r32b,tbath,bt2048,NQ_PRIMARY,NT_CONTROL,NX_DY_PRIMARY
    )
    ...
    statesP,solveP,linP=solve_config(mod6,mod7,bgs,srcP,NT_PRIMARY)
    statesT,solveT,linT=solve_config(mod6,mod7,bgs,srcT,NT_CONTROL)
    linear_max=max(linP,linT)
    state_time=state_time_control(bgs,statesP,statesT)
    conP=constraint_global(mod6,mod7,bgs,statesP,srcP,NT_PRIMARY)
    conT=constraint_global(mod6,mod7,bgs,statesT,srcT,NT_CONTROL)
    shift=max(conP["shift_global_relative_to_operator_scale"],conT["shift_global_relative_to_operator_scale"])
    aniso=max(conP["anisotropy_global_relative_to_operator_scale"],conT["anisotropy_global_relative_to_operator_scale"])
    boundary=max(zero_boundary_control(bgs,statesP,NT_PRIMARY),zero_boundary_control(bgs,statesT,NT_CONTROL))
    ...
    gates={
        ...
        "Q_cross_polarization_self_consistency_le_1e12":bool(qpolar<=Q_POLAR_MAX),
        ...
        "H4_linear_system_relative_L2_residual_le_1e8":bool(linear_max<=LINEAR_MAX),
        "H4_state_Nt64_vs_Nt128_global_relative_L2_le_5e3":bool(state_time["max"]<=STATE_TIME_MAX),
        "H4_shift_global_relative_to_operator_scale_le_1e6":bool(shift<=CONSTRAINT_MAX),
        "H4_anisotropy_global_relative_to_operator_scale_le_1e6":bool(aniso<=CONSTRAINT_MAX),
        "zero_dynamic_boundary_abs_le_1e12":bool(boundary<=BOUNDARY_MAX),
        "all_source_and_state_outputs_finite":bool(finite),
    }
    passed=bool(all(gates.values()))

    report={
        "classification":(
            "GE19_REPAIR33_WINDOW_LOCAL_REDUCED_H4_Z21_PARTICULAR_PASS"
            if passed else
            "GE19_REPAIR33_WINDOW_LOCAL_REDUCED_H4_Z21_PARTICULAR_FAIL"
        ),
        "predata_classification":"GE19_REPAIR33_PREDATA_WINDOW_LOCAL_REDUCED_H4_Z21_PARTICULAR",
        "uses_observational_data":False,
        "finite_physical_eta":False,
        "H4_equation":"L_GE06 Z21 = -2 Q_total(Z10,Z11) - 2 DY2[Z10;Z11] - 2 M1_GE05[Z20,q20] - 2 M2_GE05[(Z10,q10),(Z10,q10)]",
        "provenance":{
            "input_sha256":hashes,
        ...
        "construction":{
            ...
            "zero_homogeneous_H4_boundary":True,
        },
        "source_controls":{
            "Q_cross_polarization_self_consistency_max":qpolar,
        ...
        "solve_controls":{
            ...
            "primary_constraints":conP,
            "control_constraints":conT,
            "shift_global_relative_to_operator_scale_max":shift,
            "anisotropy_global_relative_to_operator_scale_max":aniso,
            "zero_dynamic_boundary_abs_max":boundary,
        ...
        "claim_boundary":"Repair33 certifies only the window-local particular reduced H4/Z21 state on the frozen late-time scalar/AeST/pressureless-dust scope. It does not choose a primordial homogeneous Z21 mode, certify a full-species nonlinear cosmology, introduce finite eta, or establish an observational signal."
    }
...
if __name__=="__main__":
    try:
        ...
        print(json.dumps({
            "classification":"GE19_REPAIR33_IMPLEMENTATION_FAIL",
            "error":repr(exc),
