#!/usr/bin/env python3
"""GE19 Repair02 window-retarded reduced-H3 Z20 particular solve.
...
def build_stable_ge06_generator(mod6):
    """Re-lambdify frozen GE06 partials using its captured symbolic arg tuple.

    Frozen GE06 later overwrites some module-global names (notably adot) with
    numerical deterministic-grid arrays.  Its direction_args tuple was
    created before that audit and therefore remains the authoritative symbolic
    signature.  Repair02 uses only that captured tuple.
    """
    ...
    # Frozen index contract from Repair02 predata.
    ...
    for key,expr in mod6.partial_map.items():
        ...
        coeff1[key]=sp.diff(ee,eps).subs(eps,0)
        coeff2[key]=sp.diff(ee,eps,2).subs(eps,0)
...
def main():
    ...
    if not (provenance_pass and stage_A_pass):
        result={
            "classification":"GE19_REPAIR02_WINDOW_RETARDED_REDUCED_H3_Z20_PARTICULAR_FAIL",
...
if __name__=="__main__":
    try:
        ...
        # Keep Repair02 implementation failures distinct from science FAIL.
        print(json.dumps({
            "classification":"GE19_REPAIR02_WINDOW_RETARDED_REDUCED_H3_Z20_PARTICULAR_IMPLEMENTATION_FAIL",
            "error":repr(exc),
        },indent=2))
        raise
