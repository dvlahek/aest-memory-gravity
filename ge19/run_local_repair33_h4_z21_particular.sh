...
if [[ -z "${VIRTUAL_ENV:-}" ]]; then
  echo GE19_REPAIR33_VENV_NOT_ACTIVE
...
echo GE19_REPAIR33_LOCAL_PREEXECUTION_AUDIT_PASS
...
for f in "${!HASHES[@]}"; do
  if [[ ! -f "$f" ]]; then
    echo "GE19_REPAIR33_MISSING_LOCAL_PARENT: $f"
    exit 6
  fi
...
NPZ="results/ge19_repair33_window_local_reduced_h4_z21_particular.npz"
FULL="results/ge19_repair33_window_local_reduced_h4_z21_particular_FULL.log"
rm -f "$JSON" "$NPZ" "$FULL"

set +e
python3 ge19/repair33_window_local_reduced_h4_z21_particular.py   --results-dir results   --repair26-trace "$TRACE"   --json-out "$JSON"   --npz-out "$NPZ"   2>&1 | tee "$FULL"
rc=${PIPESTATUS[0]}
set -e
...
if [[ -s "$JSON" ]]; then
  python3 - <<'PY'
import json
d=json.load(open("results/ge19_repair33_window_local_reduced_h4_z21_particular.json"))
...
if [[ "$rc" -eq 0 ]]; then
  echo GE19_REPAIR33_EXECUTION_COMPLETE
elif [[ "$rc" -eq 2 && -s "$JSON" ]]; then
  echo GE19_REPAIR33_VALID_SCIENCE_FAIL_FREEZE_REQUIRED
else
  echo GE19_REPAIR33_IMPLEMENTATION_OR_EXECUTION_FAILURE
fi
exit "$rc"
