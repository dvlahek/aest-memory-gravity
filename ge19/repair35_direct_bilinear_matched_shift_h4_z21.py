#!/usr/bin/env python3
"""GE19 Repair35 — direct-bilinear, matched-shift reduced H4/Z21 certification.
...
def build_context(rd,r13npz):
    ...
    mod6f=r7.load_frozen_generator(ROOT/"ge06/analytic_aest_directional_source_generator.py","ge06r35")
    mod6=r7.build_stable_ge06_generator_v2(mod6f)
    mod7=r7.load_frozen_generator(ROOT/"ge07/pressureless_matter_directional_source_generator.py","ge07r35")
    ge05=r7.load_frozen_generator(ROOT/"ge05/memory_directional_source_generator.py","ge05r35")
    normf,normdiag=r24.build_normalized_c2_generator(ge05)
    qdirect=build_direct_bilinear_generators(mod6f,mod6,mod7)
    return bgs,bgctl,mod6,mod7,ge05,normf,normdiag,qdirect
...
def assemble_nonmemory(qdirect,mod6,mod7,bg,tag,z10,d10,z11,d11,nx_dy):
    """Direct symbolic Q cross + unchanged DY2."""
    ...
    return (
        (qga_main,qga_con),(qm_main,qm_con),dy_by_beta,
        symmetry,cancellation_report
    )
...
def build_source_config(
    ...
    for tag in r7.C_TAGS:
        ...
        qga,qm,dy,qsymerr,qcancelerr=assemble_nonmemory(
        ...
        out[tag]=bybeta
    return out,max(qsym,default=0.0),max(qcancel,default=0.0)
...
def main():
    ...
    bgs,bgctl,mod6,mod7,ge05,normf,normdiag,qdirect=build_context(rd,r13npz)
    ...
    report={
        "classification":(
            "GE19_REPAIR35_DIRECT_BILINEAR_MATCHED_SHIFT_H4_Z21_PASS"
            if passed else
            "GE19_REPAIR35_DIRECT_BILINEAR_MATCHED_SHIFT_H4_Z21_FAIL"
        ),
        "predata_classification":"GE19_REPAIR35_PREDATA_DIRECT_BILINEAR_MATCHED_SHIFT_H4_Z21",
        ...
        "claim_boundary":"Repair35 certifies only the canonical window-local particular reduced H4/Z21 state on the frozen late-time scalar/AeST/pressureless-dust scope. It uses the exact symbolic bilinear GE06/GE07 Q cross, the Repair21/Repair22 active/near-null shift certification rule, and the unchanged Repair18/Repair07 boundary/propagator. It does not choose a primordial homogeneous Z21 mode, certify a full-species nonlinear cosmology, introduce finite eta, or establish an observational signal."
    }
...
if __name__=="__main__":
    try:
        ...
        print(json.dumps({
            "classification":"GE19_REPAIR35_IMPLEMENTATION_FAIL",
            "error":repr(exc),
