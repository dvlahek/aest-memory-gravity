#!/usr/bin/env python3
"""GE19 H3G — corrected-Y Z20-parent normalized-bath q20 science core.

Consumes frozen H3F action-completed-Y Z20; the Repair26 R1 full-history
trace is bound by the separate wrapper. All non-main physics helpers are
AST-identical to frozen Repair24. No H4/Z21 solve and no finite eta.
"""
...
def main():
    ...
    required={
        "r22j":rd/"ge19_h3f_corrected_y_z20_science_reclosure.json",
        "r22n":rd/"ge19_h3f_corrected_y_z20_science_reclosure.npz",
    ...
    hashes={
        "H3F_Z20_JSON":sha256(required["r22j"]),
        "H3F_Z20_NPZ":sha256(required["r22n"]),
        "Repair13_JSON":sha256(required["r13j"]),
        "Repair13_NPZ":sha256(required["r13n"]),
    }
    if hashes["H3F_Z20_JSON"]!=H3F_JSON_SHA: raise RuntimeError("H3F Z20 JSON hash mismatch")
    if hashes["H3F_Z20_NPZ"]!=H3F_NPZ_SHA: raise RuntimeError("H3F Z20 NPZ hash mismatch")
    ...
    if d22.get("classification")!="GE19_H3F_CORRECTED_Y_H3_Z20_CERTIFIED":
        raise RuntimeError("H3F Z20 parent not CERTIFIED")
    if d22.get("Z20_certified") is not True or any(v is not True for v in d22.get("gates",{}).values()):
        raise RuntimeError("H3F Z20 science gates not all PASS")
    ...
    # Exact certified H3F Z20 time grids.
    if aor(z22["x_primary"],bgs[(128,r7.C_TAGS[0])]["x"])>1e-14:
        raise RuntimeError("H3F primary x grid mismatch")
    if aor(z22["x_control"],bgs[(64,r7.C_TAGS[0])]["x"])>1e-14:
        raise RuntimeError("H3F control x grid mismatch")
    ...
    result={
        "classification":(
            ...
            if passed else "GE19_H3G_CORRECTED_Y_Q20_CORE_FAIL"
        ),
        "predata_classification":"GE19_H3G_PREDATA_CORRECTED_Y_Q20_RECONSTRUCTION",
        "provenance":{
            **hashes,
        ...
        "H4_Z21_Noether_precondition_still_open":True,
        "Z21_licensed_after_freeze":False,
        "claim_boundary":"PASS certifies only the independently reconstructed window-local particular normalized-bath weighted q20 response on the certified H3F corrected-Y Z20 low-mode parent. It does not certify full all-sector H4 Noether, Z21, a homogeneous bath mode, finite eta, lensing or observations.",
    }
