#!/usr/bin/env bash
# GE19 H4F3d10 local PHYSICAL original-parent first-order nonbath audit.
...
[[ "$(git symbolic-ref --quiet --short HEAD)" == "physics-first-gravitational-elasticity" ]] || {
  echo GE19_H4F3D10_WRONG_BRANCH; exit 4;
}
PINS=(
...
for entry in "${PINS[@]}"; do
  ...
  [[ "$(git rev-parse "HEAD:$rel")" == "$want" ]] || { echo "GE19_H4F3D10_BLOB_MISMATCH $rel"; exit 4; }
  git diff --quiet -- "$rel" || { echo "GE19_H4F3D10_DIRTY $rel"; exit 4; }
  git diff --cached --quiet -- "$rel" || { echo "GE19_H4F3D10_STAGED $rel"; exit 4; }
done
git diff --quiet -- ge19/run_local_h4f3d10_actual_nonbath_first_order_known_boundary.sh
git diff --cached --quiet -- ge19/run_local_h4f3d10_actual_nonbath_first_order_known_boundary.sh
echo GE19_H4F3D10_ORIGINAL_CODE_LOCK_PASS

if [[ -z "${VIRTUAL_ENV:-}" ]]; then
  [[ -f "$ROOT/.venv/bin/activate" ]] || { echo GE19_H4F3D10_VENV_MISSING; exit 5; }
...
[[ -n "$TRACE" && -s "$TRACE" ]] || { echo GE19_H4F3D10_MISSING_OR_AMBIGUOUS_R1_TRACE; exit 6; }
TRACE="$(realpath "$TRACE")"
check() {
 local path="$1" want="$2" got
 [[ -s "$path" ]] || { echo "GE19_H4F3D10_MISSING $path"; exit 6; }
 got="$(sha256sum "$path" | awk '{print $1}')"
 [[ "$got" == "$want" ]] || { echo "GE19_H4F3D10_SHA_MISMATCH $path $got"; exit 6; }
}
...
echo GE19_H4F3D10_ORIGINAL_PHYSICAL_INPUT_SHA_PASS

JSON="$ROOT/results/ge19_h4f3d10_actual_nonbath_first_order_known_boundary.json"
NPZ="$ROOT/results/ge19_h4f3d10_actual_nonbath_first_order_known_boundary.npz"
FULL="$ROOT/results/ge19_h4f3d10_actual_nonbath_first_order_known_boundary_FULL.log"
for dest in "$JSON" "$NPZ" "$FULL"; do
  [[ ! -e "$dest" ]] || { echo "GE19_H4F3D10_NO_OVERWRITE $dest"; exit 7; }
done
TMP="$(mktemp -d)"
trap 'rm -rf -- "$TMP"' EXIT
mkdir -p "$TMP/results"
...
echo GE19_H4F3D10_ISOLATED_LOCAL_EXECUTION
set +e
python3 -B -m ge19.h4f3d10_physical_main --results-dir "$ROOT/results" --repair26-trace "$TRACE" --json-out "$JSON" --npz-out "$NPZ" 2>&1 | tee "$FULL" | tail -n 28
rc="${PIPESTATUS[0]}"
set -e
...
if [[ "$rc" -eq 0 && -s "$JSON" && -s "$NPZ" ]]; then
  echo GE19_H4F3D10_ACTUAL_KNOWN_NONBATH_DIAGNOSTIC_PASS_FULL_OPEN
else
  echo GE19_H4F3D10_PHYSICAL_IMPLEMENTATION_OR_ARCHIVE_DIAGNOSTIC_FAIL
fi
exit "$rc"
