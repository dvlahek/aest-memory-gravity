...
test "$(git rev-parse HEAD:ge19/repair40_piecewise_stage_source_decomposition.py)" = "a8801d2b517540b6407a12ca2ff42774ea2b84b3"
test "$(git rev-parse HEAD:.github/workflows/ge19-repair40-prelock-audit.yml)" = "3c5ef1bd3b597c5f2da8a05937bff8daedf56c63"
...
test "$(git rev-parse HEAD:docs/ge19_repair40_piecewise_stage_source_implementation_lock.md)" = "36155cc821701e27cd9a79e6cf526174affff856"
...
echo GE19_REPAIR40_LOCAL_PREEXECUTION_AUDIT_PASS
...
echo GE19_REPAIR40_FROZEN_INPUTS_PASS
...
FULL="results/ge19_repair40_piecewise_stage_source_decomposition_FULL.log"
rm -f "$JSON" "$NPZ" "$FULL"
...
if [[ "$rc" -eq 0 && -s "$JSON" ]]; then
  echo GE19_REPAIR40_DIAGNOSTIC_COMPLETE
elif [[ "$rc" -eq 3 && -s "$JSON" ]]; then
  echo GE19_REPAIR40_IMPLEMENTATION_REPRODUCTION_FAIL
else
  echo GE19_REPAIR40_IMPLEMENTATION_OR_EXECUTION_FAILURE
fi
exit "$rc"
