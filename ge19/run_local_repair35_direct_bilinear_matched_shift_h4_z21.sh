...
test "$(git rev-parse HEAD:.github/workflows/ge19-repair35-prelock-audit.yml)" = "e8c5aee4237ff2c2d77e60727539e30d31f26cb2"
...
test "$(git rev-parse HEAD:docs/ge19_repair35_direct_bilinear_matched_shift_implementation_lock.md)" = "d64d80cfa1d0c1300f0f878a71f43ee8aa7136f3"

for c in   8bab319dccd212ed6a92f4e0f448e46b84eed5c8   17464d784fd35ef8bd6d67d4f0c6da660c20ff9a   f02508170615c32be171fa4dc4121a6b4f6c9ce6   758b2813fdc0a37d25c7eb19ba0a90ed30c3e26b   af2e3ed0e7ecf5baa01bb925536116fc379d3dbb
do
  git merge-base --is-ancestor "$c" HEAD
done
echo GE19_REPAIR35_LOCK_PASS

if [[ -z "${VIRTUAL_ENV:-}" ]]; then
  echo GE19_REPAIR35_VENV_NOT_ACTIVE
...
echo GE19_REPAIR35_LOCAL_PREEXECUTION_AUDIT_PASS
...
for f in "${!HASHES[@]}"; do
  if [[ ! -f "$f" ]]; then
    echo "GE19_REPAIR35_MISSING_LOCAL_PARENT: $f"
    exit 6
  fi
  got="$(sha256sum "$f" | awk '{print $1}')"
  test "$got" = "${HASHES[$f]}"
done
echo GE19_REPAIR35_LOCAL_PARENTS_PASS

ARTROOT="$ROOT/frozen_repair26_repair35"
TRACE="$(find "$ARTROOT" -type f -name ge19_repair26_R1_full_history_trace.dat -print -quit 2>/dev/null || true)"
if [[ -z "$TRACE" ]]; then
  command -v gh >/dev/null 2>&1 || {
    echo "GE19_REPAIR35_GH_NOT_AVAILABLE"
    exit 7
  }
  rm -rf "$ARTROOT"
  mkdir -p "$ARTROOT"
...
echo "TRACE=$TRACE"
echo GE19_REPAIR35_REPAIR26_TRACE_PASS

JSON="results/ge19_repair35_direct_bilinear_matched_shift_h4_z21.json"
NPZ="results/ge19_repair35_direct_bilinear_matched_shift_h4_z21.npz"
FULL="results/ge19_repair35_direct_bilinear_matched_shift_h4_z21_FULL.log"
rm -f "$JSON" "$NPZ" "$FULL"

set +e
python3 ge19/repair35_direct_bilinear_matched_shift_h4_z21.py   --results-dir results   --repair26-trace "$TRACE"   --json-out "$JSON"   --npz-out "$NPZ"   2>&1 | tee "$FULL"
rc=${PIPESTATUS[0]}
set -e
...
python3 - <<'PY'
import json
d=json.load(open("results/ge19_repair35_direct_bilinear_matched_shift_h4_z21.json"))
...
if [[ "$rc" -eq 0 ]]; then
  echo GE19_REPAIR35_EXECUTION_COMPLETE
elif [[ "$rc" -eq 2 && -s "$JSON" ]]; then
  echo GE19_REPAIR35_VALID_SCIENCE_FAIL_FREEZE_REQUIRED
else
  echo GE19_REPAIR35_IMPLEMENTATION_OR_EXECUTION_FAILURE
fi
exit "$rc"
