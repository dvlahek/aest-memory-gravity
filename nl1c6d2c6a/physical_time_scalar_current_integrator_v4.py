...
idx = [i for i, line in enumerate(lines) if '"E":rel_l2(' in line]
if len(idx) != 1:
    raise RuntimeError(f"expected exactly one E diagnostic line, found {len(idx)}")
lines[idx[0]] = '        "E":rel_l2(np.stack([x["E"] for x in nl["checkpoints"]]),np.stack([x["E"] for x in lin["checkpoints"]])),'
src = "\n".join(lines) + "\n"

# Technical source-load repair only. No equations, parameters, step counts,
