...
def main() -> int:
    ...
    ap.add_argument("--reference-meta", default="results/c3_r5_zero_safe_linear_reference.json")
    ap.add_argument("--json-out", default="results/c3_r5_original_c3_gate.json")
    args = ap.parse_args()
    ...
    try:
        ...
        if not audits or not all(bool(v) for v in audits.values()):
            raise RuntimeError(f"R5 reference audits not all true: {audits}")
        if meta.get("classification") != "C3_R5_ZERO_COUPLING_SAFE_REFERENCE_READY":
            raise RuntimeError("R5 reference metadata classification mismatch")
        ...
        if not linear.get("finite", False):
            raise RuntimeError(f"R5 offline linear control nonfinite: {linear.get('fail_reason')}")

        # Unchanged original D2C6C comparison function and gate.
        ...
        print(
            f"R5_C3 alpha={err['alpha']:.12e} E={err['E']:.12e} "
            f"chi={err['chi']:.12e} pass={passed}",
            flush=True,
        )
        classification = "C3_R5_ZERO_SAFE_TANGENT_PASS" if passed else "C3_R5_ZERO_SAFE_TANGENT_FAIL"
        report = {
            "classification": classification,
            "historical_d2c6c_r2_r3_r4_remain_fail": True,
        ...
        print(f"C3_R5_GATE_ERROR {type(exc).__name__}: {exc}", flush=True)
        print("CLASSIFICATION=C3_R5_REFERENCE_REPAIR_INCOMPLETE", flush=True)
        print("FINITE_POSITIVE_ETA_NONLINEAR_MEMORY_LICENSED=False", flush=True)
        return 3
