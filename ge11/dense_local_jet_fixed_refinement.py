#!/usr/bin/env python3
"""GE11 fixed two-level refinement of the frozen GE09 local-jet representation.

Runs the exact GE09 construction at two preregistered CLASS precision levels.
No interpolation-family or science-threshold change.
"""
...
LEVELS={
    "R1": ROOT/"ge11"/"pre"/"R1.pre",
    "R2": ROOT/"ge11"/"pre"/"R2.pre",
}
P3_N=[17,17,20,20,22,30]
P3_GAPS=np.asarray([
...
def run_level(class_root:Path,tag:str,pre:Path):
    dense=ROOT/"results"/f"ge11_{tag}_dense_accepted_step_trace.dat"
    source=ROOT/"results"/f"ge11_{tag}_source_state_trace.dat"
    prefix=ROOT/"results"/f"ge11_{tag}_cli"
    log=ROOT/"results"/f"ge11_{tag}_class.log"
    ...
    ini=class_root/f"ge11_{tag}.ini"
...
def main():
    ...
    refinement_gates={
        ...
        "accepted_points_R1_not_less_than_p3_each_k":bool(np.all(r1_n>=np.asarray(P3_N))),
        "accepted_points_R2_not_less_than_R1_each_k":bool(np.all(r2_n>=r1_n)),
        "max_ln_a_gap_R1_not_greater_than_p3_each_k":bool(np.all(r1_gap<=P3_GAPS*(1+1e-12))),
    ...
    result={
        ...
        "scope":"Fixed two-level factor-2 precision refinement of the unchanged GE09 accepted-endpoint cubic-Hermite local-jet representation.",
        ...
        "claim_boundary":"PASS certifies only the complete GE06 first-order local jet under the new fixed refined representation. GE08 Repair01 matter incompleteness remains and Z20 is not licensed.",
    }
