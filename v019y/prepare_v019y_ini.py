...
def rewrite(text,root,memory_enabled,order):
    ...
    out += [
        '# v0.19y eta=0 variational tangent, 17-digit CLASS output control',
...
def main():
    ...
    for label,(order,precision) in TRACE_CASES.items():
        name=f'v019y_{label}.ini';root=f'output/v019y_{label}_';(dst/name).write_text(rewrite(base,root,True,order));print(name,precision,'trace',order)
    for label,(precision,lam) in FORCE_CASES.items():
        name=f'v019y_{label}.ini';root=f'output/v019y_{label}_';(dst/name).write_text(rewrite(base,root,False,39));print(name,precision,'force',lam)
