...
if command -v python3 >/dev/null 2>&1; then
  ...
  echo "D2C6G: neither python3 nor python is installed" >&2
  exit 2
fi

VENV="$ROOT/.local/d2c6e_venv"
if [[ ! -x "$VENV/bin/python" ]]; then
  echo "D2C6G: creating isolated Python environment..."
  if ! "$BASE_PY" -m venv "$VENV"; then
    echo "D2C6G: Python venv support is missing. On Ubuntu/WSL run: sudo apt install python3-venv" >&2
    exit 2
  fi
fi
...
print("D2C6G_PYTHON_ENV_PASS")
print("Cython=", Cython.__version__)
print("numpy=", numpy.__version__)
print("scipy=", scipy.__version__)
PY
then
  echo "D2C6G: installing missing local Python dependencies..."
...
if [[ "$need_setup" -eq 1 ]]; then
  echo "D2C6G: building local zero-safe CLASS once..."
  bash nl1c6d2c6c_r5/setup_zero_safe_class.sh
  # shellcheck disable=SC1091
  source results/c3_r5_class_env.sh
else
  echo "D2C6G: reusing local zero-safe CLASS at ${C3_R5_CLASS_ROOT}"
fi

python -m py_compile nl1c6d2c6g/eta0_metric_tangent_calibration.py

LOG="results/nl1c6d2c6g_eta0_metric_tangent_calibration.log"
JSON="results/nl1c6d2c6g_eta0_metric_tangent_calibration.json"
ZIP="results/nl1c6d2c6g_local_bundle.zip"
rm -f "$LOG" "$JSON" "$ZIP"

set +e
python -u nl1c6d2c6g/eta0_metric_tangent_calibration.py --json-out "$JSON" 2>&1 | tee "$LOG"
code=${PIPESTATUS[0]}
set -e
...
paths=[
    ...
    Path("nl1c6d2c6g/run_local.sh"),
    Path("nl1c6d2c6g/README_LOCAL.md"),
...
print(f"D2C6G_BUNDLE={zip_path}")
PY

echo "D2C6G_EXIT=$code"
echo "D2C6G_LOG=$LOG"
echo "D2C6G_JSON=$JSON"
echo "D2C6G_ZIP=$ZIP"
exit "$code"
