...
else echo "STABLE_AEST_FINITE_MEMORY_R1B: python missing" >&2; exit 2; fi

VENV="$ROOT/.local/fullj_weyl_bridge_venv"
if [[ ! -x "$VENV/bin/python" ]]; then
  "$BASE_PY" -m venv "$VENV" || { echo "STABLE_AEST_FINITE_MEMORY_R1B: install python3-venv" >&2; exit 2; }
fi
export PATH="$VENV/bin:$PATH"
python -m pip install --upgrade pip setuptools wheel >/dev/null
python -m pip install numpy scipy cython >/dev/null
python -m py_compile fullj_weyl/stable_aest_finite_memory_r1b.py fullj_weyl/stable_aest_finite_memory_r1.py fullj_weyl/apply_aest_stable_chi_residual_patch.py

echo "STABLE_AEST_FINITE_MEMORY_R1B_IMPORT_PASS"

for f in \
  docs/stable_aest_finite_memory_r1b_predata.md \
  docs/stable_aest_finite_memory_r1_postdata.md \
  results/stable_aest_finite_memory_r1.json \
  results/stable_aest_finite_memory_r1.npz \
  results/fullj_aest_stable_chi_precision_floor.json \
  results/nl1c6d2n_corrected_class_densek64_env.sh; do
  [[ -f "$f" ]] || { echo "STABLE_AEST_FINITE_MEMORY_R1B: missing $f" >&2; exit 3; }
done
...
echo "STABLE_AEST_FINITE_MEMORY_R1B_SCIENCE_LOCK_PASS predata=$PREDATA_LOCK postdata=$POSTDATA_LOCK"

python - <<'PY'
import json
from pathlib import Path
r=json.loads(Path('results/stable_aest_finite_memory_r1.json').read_text())
h=json.loads(Path('results/fullj_aest_stable_chi_precision_floor.json').read_text())
assert r['classification']=='STABLE_AEST_FINITE_MEMORY_R1_ETA_SMOOTHNESS_FAIL',r['classification']
assert r['diagnostic_complete'] is True
assert h['classification']=='FULLJ_AEST_STABLE_CHI_PRECISION_FLOOR_CERTIFIED',h['classification']
assert h['diagnostic_complete'] is True
print('STABLE_AEST_FINITE_MEMORY_R1B_PARENT_PASS')
PY
...
echo "STABLE_AEST_FINITE_MEMORY_R1B_OLD_PROVENANCE_PASS head=$EXPECTED_CLASS_HEAD"
...
if [[ "$need_build" -eq 1 ]]; then
  echo "STABLE_AEST_FINITE_MEMORY_R1B: preparing stable-residual CLASS copy..."
  ...
  echo "STABLE_AEST_FINITE_MEMORY_R1B: reusing stable-residual CLASS copy"
fi
...
echo "STABLE_AEST_FINITE_MEMORY_R1B_SOURCE_PASS"
...
JSON="results/stable_aest_finite_memory_r1b.json"
NPZ="results/stable_aest_finite_memory_r1b.npz"
LOG="results/stable_aest_finite_memory_r1b.log"
ZIP="results/stable_aest_finite_memory_r1b_bundle.zip"
rm -f "$JSON" "$NPZ" "$LOG" "$ZIP"

set +e
PYTHONPATH="$STABLE_PYTARGET" python -u -m fullj_weyl.stable_aest_finite_memory_r1b \
...
zp=Path(sys.argv[1]); paths=[Path(x) for x in sys.argv[2:]]+[
 Path('docs/stable_aest_finite_memory_r1b_predata.md'),
 Path('docs/stable_aest_finite_memory_r1_postdata.md'),
 Path('fullj_weyl/stable_aest_finite_memory_r1b.py'),
 Path('fullj_weyl/run_local_stable_aest_finite_memory_r1b.sh'),
 Path('fullj_weyl/stable_aest_finite_memory_r1.py'),
 Path('fullj_weyl/apply_aest_stable_chi_residual_patch.py'),
 Path('results/stable_aest_finite_memory_r1.json'),
 Path('results/stable_aest_finite_memory_r1.npz'),
...
print('STABLE_AEST_FINITE_MEMORY_R1B_BUNDLE='+str(zp))
PY

echo "STABLE_AEST_FINITE_MEMORY_R1B_EXIT=$code"
echo "STABLE_AEST_FINITE_MEMORY_R1B_LOG=$LOG"
echo "STABLE_AEST_FINITE_MEMORY_R1B_JSON=$JSON"
echo "STABLE_AEST_FINITE_MEMORY_R1B_NPZ=$NPZ"
echo "STABLE_AEST_FINITE_MEMORY_R1B_ZIP=$ZIP"
exit "$code"
