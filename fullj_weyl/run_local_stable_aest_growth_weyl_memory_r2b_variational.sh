...
[[ -n "$BASE_PY" ]] || { echo 'STABLE_AEST_GROWTH_WEYL_MEMORY_R2B: python missing' >&2; exit 2; }
...
python -m py_compile \
  ...
  fullj_weyl/stable_aest_growth_weyl_memory_r2b_variational.py \
  fullj_weyl/apply_stable_aest_r2b_variational_patch.py \
  fullj_weyl/apply_aest_stable_chi_residual_patch.py \
  v019w/apply_variational_forcing_patch.py v019w/normalize_force_table.py
echo STABLE_AEST_GROWTH_WEYL_MEMORY_R2B_IMPORT_PASS

for f in \
  docs/stable_aest_growth_weyl_memory_r2b_variational_predata.md \
  docs/stable_aest_growth_weyl_memory_r2a_postdata.md \
  results/stable_aest_growth_weyl_memory_r2a.json \
  ...
  [[ -f "$f" ]] || { echo "STABLE_AEST_GROWTH_WEYL_MEMORY_R2B: missing $f" >&2; exit 3; }
done
...
git merge-base --is-ancestor "$R2A_POSTDATA_LOCK" HEAD
echo "STABLE_AEST_GROWTH_WEYL_MEMORY_R2B_SCIENCE_LOCK_PASS predata=$PREDATA_LOCK r2a_postdata=$R2A_POSTDATA_LOCK"

python - <<'PY'
import json
from pathlib import Path
r2a=json.loads(Path('results/stable_aest_growth_weyl_memory_r2a.json').read_text())
...
assert r2a['classification']=='STABLE_AEST_GROWTH_WEYL_MEMORY_R2A_SEPARATION_UNRESOLVED',r2a['classification']
...
print('STABLE_AEST_GROWTH_WEYL_MEMORY_R2B_PARENT_PASS')
PY
...
echo "STABLE_AEST_GROWTH_WEYL_MEMORY_R2B_OLD_PROVENANCE_PASS head=$EXPECTED_CLASS_HEAD"
...
if [[ ! -d "$STABLE_ROOT/.git" ]] || ! grep -q 'FULLJ_AEST_STABLE_CHI_RESIDUAL_V1' "$STABLE_ROOT/source/perturbations.c"; then
  echo 'STABLE_AEST_GROWTH_WEYL_MEMORY_R2B: preparing stable-residual parent copy...'
  rm -rf "$STABLE_ROOT"
...
rm -rf "$VAR_ROOT" "$VAR_PYTARGET"
cp -a "$STABLE_ROOT" "$VAR_ROOT"
...
grep -q 'AEST_TANGENT_FORCE_FILE' "$VAR_ROOT/source/aest_memory.c"
grep -q 'E_rhs_aest -= 0.5\*Q_aest\*Bchi_aest' "$VAR_ROOT/source/perturbations.c"
if grep -q 'Q_aest\*(a\*theta_aest/(k\*k)+alpha_aest)' "$VAR_ROOT/source/perturbations.c"; then
  echo 'STABLE_AEST_GROWTH_WEYL_MEMORY_R2B: forbidden subtraction trace remains' >&2; exit 4
fi
echo STABLE_AEST_GROWTH_WEYL_MEMORY_R2B_SOURCE_PASS
...
JSON='results/stable_aest_growth_weyl_memory_r2b_variational.json'
NPZ='results/stable_aest_growth_weyl_memory_r2b_variational.npz'
LOG='results/stable_aest_growth_weyl_memory_r2b_variational.log'
ZIP='results/stable_aest_growth_weyl_memory_r2b_variational_bundle.zip'
WORK='results/stable_aest_growth_weyl_memory_r2b_work'
rm -f "$JSON" "$NPZ" "$LOG" "$ZIP"
rm -rf "$WORK"

set +e
PYTHONPATH="$VAR_PYTARGET" python -u -m fullj_weyl.stable_aest_growth_weyl_memory_r2b_variational \
...
paths=[Path(x) for x in sys.argv[2:5]]+[
 Path('docs/stable_aest_growth_weyl_memory_r2b_variational_predata.md'),
 Path('docs/stable_aest_growth_weyl_memory_r2a_postdata.md'),
 Path('fullj_weyl/stable_aest_growth_weyl_memory_r2b_variational.py'),
 Path('fullj_weyl/apply_stable_aest_r2b_variational_patch.py'),
 Path('fullj_weyl/run_local_stable_aest_growth_weyl_memory_r2b_variational.sh'),
 Path('v019w/apply_variational_forcing_patch.py'),Path('v019w/normalize_force_table.py'),
 Path('results/stable_aest_growth_weyl_memory_r2a.json'),
 Path('results/stable_aest_finite_memory_r1c.json'),Path('results/fullj_aest_stable_chi_precision_floor.json'),
 Path('results/stable_aest_growth_weyl_memory_r2.npz'),Path('results/nl1c6d2n_corrected_class_densek64_env.sh')]
...
print('STABLE_AEST_GROWTH_WEYL_MEMORY_R2B_BUNDLE='+str(zp))
PY

echo "STABLE_AEST_GROWTH_WEYL_MEMORY_R2B_EXIT=$code"
echo "STABLE_AEST_GROWTH_WEYL_MEMORY_R2B_LOG=$LOG"
echo "STABLE_AEST_GROWTH_WEYL_MEMORY_R2B_JSON=$JSON"
echo "STABLE_AEST_GROWTH_WEYL_MEMORY_R2B_NPZ=$NPZ"
echo "STABLE_AEST_GROWTH_WEYL_MEMORY_R2B_ZIP=$ZIP"
exit "$code"
