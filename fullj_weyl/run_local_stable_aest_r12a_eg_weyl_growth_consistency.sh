...
for f in "$PREFIT" "$IMPL" "$R8A2_JSON" "$R8A2_NPZ" "$R11A_R02_JSON"; do
  [[ -f "$f" ]] || { echo "STABLE_AEST_R12A_MISSING file=$f" >&2; exit 3; }
done
for lock in "$PREFIT_LOCK" "$IMPLEMENTATION_LOCK" "$R8A2_POSTDATA_LOCK" "$R10A_POSTDATA_LOCK" "$R11A_R02_POSTDATA_LOCK"; do
  git merge-base --is-ancestor "$lock" HEAD || { echo "STABLE_AEST_R12A_LOCK_FAIL lock=$lock" >&2; exit 3; }
done
[[ "$(git hash-object "$IMPL")" == "$IMPLEMENTATION_BLOB" ]] || { echo STABLE_AEST_R12A_IMPLEMENTATION_BLOB_FAIL >&2; exit 3; }
[[ "$(sha256sum "$R8A2_JSON" | awk '{print $1}')" == "$R8A2_JSON_SHA" ]] || { echo STABLE_AEST_R12A_R8A2_JSON_HASH_FAIL >&2; exit 3; }
[[ "$(sha256sum "$R8A2_NPZ" | awk '{print $1}')" == "$R8A2_NPZ_SHA" ]] || { echo STABLE_AEST_R12A_R8A2_NPZ_HASH_FAIL >&2; exit 3; }
[[ "$(sha256sum "$R11A_R02_JSON" | awk '{print $1}')" == "$R11A_R02_JSON_SHA" ]] || { echo STABLE_AEST_R12A_R11A_JSON_HASH_FAIL >&2; exit 3; }
echo "STABLE_AEST_R12A_LOCK_PASS prefit=$PREFIT_LOCK implementation=$IMPLEMENTATION_LOCK r8a2=$R8A2_POSTDATA_LOCK r10a=$R10A_POSTDATA_LOCK r11a_r02=$R11A_R02_POSTDATA_LOCK"

BASE_PY="$(command -v python3 || command -v python || true)"
[[ -n "$BASE_PY" ]] || { echo 'STABLE_AEST_R12A: python missing' >&2; exit 2; }
...
echo STABLE_AEST_R12A_IMPORT_PASS

python - <<'PY'
from pathlib import Path
p=Path('fullj_weyl/stable_aest_r12a_eg_weyl_growth_consistency.py').read_text()
required=[
 'PREFIT_LOCK = "bb69d8ac1d83228ba879316c61141823c3e5c665"',
 'R11A_R02_POSTDATA_LOCK = "d9e2e0e6da65126a81d02f32e65f83410bc8cc7c"',
...
print('STABLE_AEST_R12A_ANTI_STALE_CODE_PASS')
PY
...
print('STABLE_AEST_R12A_PARENT_PASS')
PY

# Reuse the exact R8a2 patched CLASS build. Project policy keeps .local checkpoints/builds.
...
[[ -d "$R8A2_ROOT" && -d "$R8A2_PYTARGET" ]] || {
  echo "STABLE_AEST_R12A_R8A2_CLASS_BUILD_MISSING root=$R8A2_ROOT pytarget=$R8A2_PYTARGET" >&2
...
[[ "$(git -C "$R8A2_ROOT" rev-parse HEAD)" == "$EXPECTED_CLASS_HEAD" ]] || { echo STABLE_AEST_R12A_CLASS_HEAD_FAIL >&2; exit 3; }
...
echo STABLE_AEST_R12A_SOURCE_PASS
...
print('STABLE_AEST_R12A_CLASSY_IMPORT_PASS')
PY
...
set +e
PYTHONPATH="$R8A2_PYTARGET${PYTHONPATH:+:$PYTHONPATH}" python -u -m fullj_weyl.stable_aest_r12a_eg_weyl_growth_consistency \
  --workdir "$WORK" --json-out "$JSON" --npz-out "$NPZ" 2>&1 | tee "$LOG"
code=${PIPESTATUS[0]}
set -e

echo "STABLE_AEST_R12A_EXIT=$code"
[[ -f "$JSON" ]] && sha256sum "$JSON"
[[ -f "$NPZ" ]] && sha256sum "$NPZ"
sha256sum "$LOG" "$ENVOUT"
echo "STABLE_AEST_R12A_JSON=$JSON"
echo "STABLE_AEST_R12A_NPZ=$NPZ"
echo "STABLE_AEST_R12A_LOG=$LOG"
exit "$code"
