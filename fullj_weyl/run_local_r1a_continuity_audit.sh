...
if command -v python3 >/dev/null 2>&1; then
  ...
  echo "FULLJ_WEYL_R1A: python missing" >&2
  exit 2
fi

VENV="$ROOT/.local/fullj_weyl_bridge_venv"
if [[ ! -x "$VENV/bin/python" ]]; then
  "$BASE_PY" -m venv "$VENV" || { echo "FULLJ_WEYL_R1A: install python3-venv" >&2; exit 2; }
fi
...
python -m py_compile \
  fullj_weyl/r1a_continuity_identity_audit.py \
...
if [[ "$reuse" -eq 1 ]]; then
  echo "FULLJ_WEYL_R1A: reusing existing D2C6-certified corrected CLASS environment..."
else
  echo "FULLJ_WEYL_R1A: preparing D2C6-certified corrected CLASS environment..."
...
JSON="results/fullj_weyl_r1a_continuity_audit.json"
CSV="results/fullj_weyl_r1a_continuity_audit.csv"
LOG="results/fullj_weyl_r1a_continuity_audit.log"
ZIP="results/fullj_weyl_r1a_continuity_audit_bundle.zip"
...
set +e
python -u fullj_weyl/r1a_continuity_identity_audit.py \
  --json-out "$JSON" \
...
paths=[Path(x) for x in sys.argv[2:]] + [
    ...
    Path('fullj_weyl/r1a_continuity_identity_audit.py'),
    Path('fullj_weyl/run_local_r1a_continuity_audit.sh'),
...
print('FULLJ_WEYL_R1A_BUNDLE='+str(zp))
PY

echo "FULLJ_WEYL_R1A_EXIT=$code"
echo "FULLJ_WEYL_R1A_LOG=$LOG"
echo "FULLJ_WEYL_R1A_JSON=$JSON"
echo "FULLJ_WEYL_R1A_CSV=$CSV"
echo "FULLJ_WEYL_R1A_ZIP=$ZIP"
exit "$code"
