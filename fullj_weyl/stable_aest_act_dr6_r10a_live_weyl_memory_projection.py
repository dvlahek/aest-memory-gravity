...
CLS_INCOMPLETE = "STABLE_AEST_ACT_DR6_R10A_INCOMPLETE"
...
def main() -> int:
    ...
    ap.add_argument(
        "--json-out",
        default="results/stable_aest_act_dr6_r10a_live_weyl_memory_projection.json",
    )
    ap.add_argument(
        "--npz-out",
        default="results/stable_aest_act_dr6_r10a_live_weyl_memory_projection.npz",
    )
    args = ap.parse_args()

    print("STABLE_AEST_ACT_DR6_R10A_START", flush=True)

    # G1: frozen parent provenance. R6a is an interface parent only; R8a2 is
    # the sole theory-response parent.
    parent_meta = {}
    r8j = {}
    q = None
    g1 = False
    if R8A2_JSON.is_file() and R8A2_NPZ.is_file():
        jsha = sha256(R8A2_JSON)
        nsha = sha256(R8A2_NPZ)
        ...
        g1 = bool(
            ...
            and ancestor(R6A_POSTDATA_LOCK)
            and ancestor(R8A2_POSTDATA_LOCK)
            and jsha == R8A2_JSON_SHA256
            and nsha == R8A2_NPZ_SHA256
        ...
        parent_meta = {
            "missing_r8a2_json": not R8A2_JSON.is_file(),
            "missing_r8a2_npz": not R8A2_NPZ.is_file(),
        }
    print("STABLE_AEST_ACT_DR6_R10A_PARENT " + json.dumps({"g1": g1, **parent_meta}, sort_keys=True), flush=True)

    # G2: exact official ACT interface, independently controlled exactly as R6a.
    interface_meta = {}
    data_dict = None
    ...
    if g1:
        try:
            ...
            source_ok = bool(
                source_head == ACT_COMMIT
                and internal_version == ACT_INTERNAL_VERSION
                and module_in_source
                and ddir.is_dir()
            ...
            g2 = bool(
                source_ok
                and np.isfinite(control_chi2)
                and abs(control_chi2 - CONTROL_CHI2) <= CONTROL_TOL
            )
            interface_meta = {
                "act_source_head": source_head,
                "act_internal_version": internal_version,
                "act_module": str(Path(alike.__file__).resolve()),
                "module_in_source": module_in_source,
                "data_dir": str(ddir),
    ...
    if g2 and q is not None and data_dict is not None:
        ...
        for tau in TAUS:
            tag = tau_tag(tau)
            keys = {
                "c0": f"baseline_tau{tag}_ckk",
                "t25": f"T_tau{tag}_ckk_eps025",
                "t50": f"T_tau{tag}_ckk_eps05",
            }
            if not all(k in q.files for k in keys.values()):
                required_arrays = False
                continue
            c0 = np.asarray(q[keys["c0"]], float)
            T25 = np.asarray(q[keys["t25"]], float)
            T50 = np.asarray(q[keys["t50"]], float)
            finite_positive &= bool(
                c0.shape == ell.shape
                and T25.shape == ell.shape
                and T50.shape == ell.shape
                and np.all(np.isfinite(c0))
                and np.all(c0 > 0.0)
                and np.all(np.isfinite(T25))
                and np.all(np.isfinite(T50))
            )
            if baseline_ref is None:
                baseline_ref = c0
            berr = float(np.max(np.abs(c0 - baseline_ref) / np.maximum(np.abs(baseline_ref), 1e-300)))
        ...
        g3 = bool(
            exact_ell
            and required_arrays
            and len(live) == len(TAUS)
            and finite_positive
            and nL == TRIM_LMAX + 2
            and np.all(np.isfinite(B))
            and support_frac <= SUPPORT_TOL
            and max_baseline_error <= BASELINE_TAU_TOL
        )
        support_meta = {
            "exact_ell_40_2000": exact_ell,
            "required_arrays_present": required_arrays,
            "live_arrays_finite_baseline_positive": finite_positive,
            "binmat_shape": list(B.shape),
            "support_fraction_outside_40_2000": support_frac,
            "support_threshold": SUPPORT_TOL,
            "baseline_tau_max_relative_errors_vs_tau10": baseline_errors,
            "baseline_tau_max_relative_error": max_baseline_error,
            "baseline_tau_threshold": BASELINE_TAU_TOL,
        ...
        if g3:
            ...
            for tau in TAUS:
                ...
                d25_full = np.zeros(nL, float)
                d50_full = np.zeros(nL, float)
                c0_full[idx] = c0
                d25_full[idx] = c0 * T25
                d50_full[idx] = c0 * T50
                b0 = B @ c0_full
                ...
                E = rel(t25, t50)
                C = cosine(t25, t50)
                norm25 = float(np.linalg.norm(t25))
                ok = bool(
        ...
        if g4:
            ...
            for tau in TAUS:
                ...
                orth = (
                    abs(dotc(a, tperp, cinv)) / math.sqrt(aa * Fperp)
                    if aa > 0.0 and Fperp > 0.0
                    else float("inf")
                )
                physical_positive = True
                for eta in ETA_GRID:
                    spec = c0 * (1.0 + float(eta) * T25)
                ...
                ok = bool(
                    ...
                    and aa > 0.0
                    and tt > 0.0
                    and np.isfinite(Fperp)
                    and Fperp > 0.0
                    and np.isfinite(orth)
                    and orth <= ALG_TOL
                ...
                projections[str(tau)] = {
                    ...
                    "projection_algebra_pass": ok,
                }
                arrays[f"act_tperp_tau{tau_tag(tau)}"] = tperp
            g5 = bool(algebra_all)
        ...
        if g5:
            ...
            for tau in TAUS:
                ...
                aa = live[tau]["aa"]
                tt = live[tau]["tt"]
                tperp = live[tau]["tperp"]
                Fperp = live[tau]["Fperp"]
                r = data - b0
                eta_hat = dotc(tperp, r, cinv) / Fperp
                sigma_eta = 1.0 / math.sqrt(Fperp)
                signed_sn = eta_hat / sigma_eta
                ...
                try:
                    ...
                    sa = math.sqrt(aa)
                    st = math.sqrt(tt)
                    Xs = np.column_stack([a / sa, t25 / st])
                ...
                chi2_profiled = {}
                for eta in ETA_GRID:
                    rr = data - (b0 + float(eta) * t25)
                    chi2_fixed[f"{eta:g}"] = qform(rr, cinv)
                    deltaA = dotc(a, r - float(eta) * t25, cinv) / aa
                    rr_prof = r - deltaA * a - float(eta) * t25
                ...
                chi2_phys = qform(rr_phys, cinv)
                delta_chi2_phys = chi2_profiled["0"] - chi2_phys

                projections[str(tau)].update(
                    {
                        "eta_hat_signed": eta_hat,
                        "sigma_eta": sigma_eta,
                        "signed_template_sn": signed_sn,
                        "eta_gls": eta_gls,
                        "deltaA_gls": deltaA_gls,
                        "matched_filter_gls_abs_diff": diff,
                        "matched_filter_gls_pass": gls_ok,
                        "eta_physical_clipped": eta_phys,
                        "delta_chi2_physical": delta_chi2_phys,
    ...
    if not g1:
        ...
        classification = CLS_INTERFACE
    elif not g3:
        classification = CLS_SUPPORT
    elif not g4:
        classification = CLS_CENTRAL
    elif not g5:
        classification = CLS_ALG
    elif not g6:
    ...
    out = {
        ...
        "claim_scope": {
            ...
            "tau_bound": False,
            "full_cosmological_inference": False,
            "full_ACT_primary_CMB_likelihood": False,
            "nonlinear_lensing_claim": False,
        },
        "frozen": {
            ...
            "physical_eta_interval": [0.0, 0.05],
            "primary_epsilon": 0.025,
            "control_epsilon": 0.05,
            "act_commit": ACT_COMMIT,
            "act_data_version": ACT_DATA_VERSION,
        ...
        "central_derivative_ACT_space": central_metrics,
        "projection": projections,
        "tau_coherence_ACT_space": tau_coherence,
    }
    ...
    if projections:
        summary = {
            tau: {
                k: projections[tau].get(k)
                for k in (
                    "eta_hat_signed",
                    "sigma_eta",
                    "signed_template_sn",
                    "eta_physical_clipped",
                    "delta_chi2_physical",
                    "F_perp_over_F_raw",
                )
            }
