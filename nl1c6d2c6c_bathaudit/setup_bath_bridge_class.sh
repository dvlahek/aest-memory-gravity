...
CLASS_ROOT="${CLASS_ROOT:-$ROOT/.local/class_c3_bathbridge_e8580832}"
PYTARGET="${PYTARGET:-$ROOT/.local/classy_c3_bathbridge_e8580832}"
...
python "$ROOT/nl1c6d2c6c_r4ref/apply_dense_force_output_patch.py" "$CLASS_ROOT"
python "$ROOT/nl1c6d2c6c_bathaudit/apply_bath_state_output_patch.py" "$CLASS_ROOT"

grep -q 'mem_q_aest_00' "$CLASS_ROOT/source/perturbations.c"
grep -q 'mem_q_aest_38' "$CLASS_ROOT/source/perturbations.c"
grep -q 'mem_p_aest_00' "$CLASS_ROOT/source/perturbations.c"
grep -q 'mem_p_aest_38' "$CLASS_ROOT/source/perturbations.c"
grep -q 'Bchi_aest \*= pba->aest_eta' "$CLASS_ROOT/source/perturbations.c"
...
import classy
print("C3_BATH_BRIDGE_CLASSY_IMPORT_PASS")
print("classy_module=", classy.__file__)
c=Class(); c.empty()
PY

cat > "$ROOT/results/c3_bath_bridge_class_env.sh" <<EOF
...
printf 'CLASS_ROOT=%s\nPYTARGET=%s\n' "$CLASS_ROOT" "$PYTARGET"
printf 'ENV_FILE=%s\n' "$ROOT/results/c3_bath_bridge_class_env.sh"
