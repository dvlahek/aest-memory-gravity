...
def main():
    ...
    ap.add_argument('--tauH0',type=float,required=True)
    ap.add_argument('--out-prefix',required=True)
    ap.add_argument('--control-order',type=int,default=512)
    ap.add_argument('--primary-order',type=int,default=1024)
    ap.add_argument('--summary',required=True)
    a=ap.parse_args()
    if not (a.KB>0 and a.tauH0>0): raise SystemExit('KB and tauH0 must be positive')
    ...
    for k in sorted(histories):
        ...
        yc=b.simulate_response(N,H,x,a.tauH0,a.control_order)
        yp=b.simulate_response(N,H,x,a.tauH0,a.primary_order)
    ...
    res={'classification':'V039_KB_LOCKED_POSITIVE_DRUDE_TAU_GENERALITY','KB':a.KB,'tauH0':a.tauH0,
         'control_order':a.control_order,'primary_order':a.primary_order,
         'relative_L2_control_vs_primary':rel,'cosine':cos,
         'gate':bool(rel<0.01 and cos>0.9999),'samples':len(rprows),'k_histories':len(histories)}
