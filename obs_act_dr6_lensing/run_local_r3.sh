...
if command -v python3 >/dev/null 2>&1; then
  ...
  echo "ACT_DR6_R3: neither python3 nor python is installed" >&2
  exit 2
fi
...
if [[ ! -x "$VENV/bin/python" ]]; then
  "$BASE_PY" -m venv "$VENV" || {
    echo "ACT_DR6_R3: Python venv support missing; install python3-venv" >&2
    exit 2
  }
fi
...
if [[ "$need_setup" -eq 1 ]]; then
  echo "ACT_DR6_R3: building pinned zero-safe AeST+memory CLASS..."
  bash nl1c6d2c6c_r5/setup_zero_safe_class.sh
  # shellcheck disable=SC1091
  source results/c3_r5_class_env.sh
else
  echo "ACT_DR6_R3: reusing CLASS at ${C3_R5_CLASS_ROOT}"
fi
...
python -m py_compile obs_act_dr6_lensing/act_dr6_exploratory_scan_r2.py
python -m py_compile obs_act_dr6_lensing/act_dr6_exploratory_scan_r3.py

LOG="results/act_dr6_lensing_exploratory_r3.log"
JSON="results/act_dr6_lensing_exploratory_r3.json"
NPZ="results/act_dr6_lensing_exploratory_r3.npz"
ZIP="results/act_dr6_lensing_exploratory_r3_bundle.zip"
rm -f "$LOG" "$JSON" "$NPZ" "$ZIP"

set +e
python -u obs_act_dr6_lensing/act_dr6_exploratory_scan_r3.py \
...
paths=[
    ...
    Path('docs/obs_act_dr6_lensing_exploratory_r3_nonfinite_lowell_repair.md'),
    Path('obs_act_dr6_lensing/act_dr6_exploratory_scan.py'),
    Path('obs_act_dr6_lensing/act_dr6_exploratory_scan_r2.py'),
    Path('obs_act_dr6_lensing/act_dr6_exploratory_scan_r3.py'),
    Path('obs_act_dr6_lensing/run_local_r3.sh'),
...
print(f'ACT_DR6_R3_BUNDLE={zip_path}')
PY

echo "ACT_DR6_R3_EXIT=$code"
echo "ACT_DR6_R3_LOG=$LOG"
echo "ACT_DR6_R3_JSON=$JSON"
echo "ACT_DR6_R3_NPZ=$NPZ"
echo "ACT_DR6_R3_ZIP=$ZIP"
exit "$code"
