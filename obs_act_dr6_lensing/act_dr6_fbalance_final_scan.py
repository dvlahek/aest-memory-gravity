...
PREDATA = "2d31fe438f5916f7ed0d302569f4750d66e62ee8"
...
COMPLETE = "ACT_DR6_FBALANCE_CERTIFIED_LINEAR_ETA_SCAN_COMPLETE"
CERT_FAIL = "ACT_DR6_FBALANCE_CERTIFICATION_FAIL"
INCOMPLETE = "ACT_DR6_FBALANCE_INCOMPLETE"
...
def make_ini(text,root,memory,eta):
    ...
    out += ["# F-balance final linear ACT route","aest_enabled = yes",
...
def run_class(cr,label,memory,eta,kmax):
    ini=cr/f"obs_act_fb_{label}.ini"; root=f"output/obs_act_fb_{label}_"
    ini.write_text(make_ini(BASE_INI.read_text(),root,memory,eta))
    shutil.copy2(ini,ROOT/"results"/f"act_dr6_fb_{label}.ini")
    ...
    for n in ("AEST_TANGENT_FORCE_FILE","AEST_TANGENT_LAMBDA","AEST_TANGENT_TRACE_FILE","AEST_OFFLINE_TRACE_FILE","AEST_TANGENT_ALLOW_K_MISS"):
        env.pop(n,None)
    log=ROOT/"results"/f"act_dr6_fb_class_{label}.log"
    ...
    src=cr/"output"/f"obs_act_fb_{label}__cl.dat"
    if not src.exists() or not src.stat().st_size: raise RuntimeError(f"missing _cl.dat for {label}")
    dst=ROOT/"results"/f"act_dr6_fb_{label}__cl.dat"; shutil.copy2(src,dst); return dst
...
def load_ckk(path,label):
    ...
    print(f"FB_CKK_FINITE label={label} N={ell.size} L=2..{ACT_REQUIRED_LMAX} min={s['ckk_min']:.12e} max={s['ckk_max']:.12e}",flush=True)
    return c,s
...
def save_fail(jout,nout,h,anc,cert,obs,arrays):
    result={"classification":CERT_FAIL,"observational_claim_licensed":False,
            ...
            "class_upstream_sha":CLASS_SHA,"certification":cert,"eta_scan_executed":False}
...
def main():
    ...
    ap.add_argument("--json-out",default="results/act_dr6_fbalance_certified_eta_scan.json")
    ap.add_argument("--npz-out",default="results/act_dr6_fbalance_certified_eta_scan.npz")
    ...
    try:
        ...
        h=git_head(); anc={"r9_base":ancestor(R9_BASE),"fbalance_predata":ancestor(PREDATA)}
        print("ACT_DR6_FBALANCE_FINAL_START",flush=True); print("HEAD="+h,flush=True); print("ANCESTRY="+json.dumps(anc,sort_keys=True),flush=True)
        print(f"MODEL AeST Exp KB={KB} tauH0={TAUH0} memory_order={MEMORY_ORDER}",flush=True)
        print("NUMERICAL_REPRESENTATION=direct_F_balance",flush=True)
        ...
        # eta=0 identity: memory-off and memory-on must coincide.
        ...
        print(f"FB_ETA0_REG relL2={eta0rel:.12e} gate={ETA0_GATE:.1e} pass={eta0pass}",flush=True)

        # Same high-k convergence gate that R9 failed.
        ...
        print(f"FB_KMAX_CERT chi2_k24={k24chi:.12e} chi2_k30={onchi:.12e} binned_relL2={krel:.12e} gate={KMAX_GATE:.1e} pass={kpass}",flush=True)
        ...
        if not cert["pass"]:
            ...
            print("FB_CERTIFICATION=FAIL",flush=True); print("CLASSIFICATION="+CERT_FAIL,flush=True); return 2
        print("FB_CERTIFICATION=PASS",flush=True)
        ...
        for i,eta in enumerate(ETA_GRID):
            ...
            print(f"FB_ACT_POINT {i+1:02d}/{len(ETA_GRID):02d} eta={eta:.12e} chi2={chi:.12e} relL2={shift:.12e}",flush=True)
        chi0=rows[0]["chi2"]
        ...
        result={"classification":COMPLETE,"observational_claim_licensed":False,"act_physics_interpretation_licensed":True,"exploratory":True,"eta_scan_executed":True,
                "head":h,"ancestry":anc,"class_upstream_sha":CLASS_SHA,"numerical_representation":"direct_F_balance",
        ...
        print("FB_BEST "+json.dumps(bests,sort_keys=True),flush=True); print("CLASSIFICATION="+COMPLETE,flush=True); print("OBSERVATIONAL_CLAIM_LICENSED=False",flush=True); return 0
        ...
        jout.write_text(json.dumps(result,indent=2,sort_keys=True)+"\n"); print("FB_ERROR="+result["error"],flush=True); print("CLASSIFICATION="+INCOMPLETE,flush=True); return 1
