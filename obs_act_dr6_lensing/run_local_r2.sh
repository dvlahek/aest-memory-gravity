...
if command -v python3 >/dev/null 2>&1; then
  ...
  echo "ACT_DR6_R2: neither python3 nor python is installed" >&2
  exit 2
fi
...
if [[ ! -x "$VENV/bin/python" ]]; then
  "$BASE_PY" -m venv "$VENV" || {
    echo "ACT_DR6_R2: Python venv support missing; install python3-venv" >&2
    exit 2
  }
fi
...
if [[ "$need_setup" -eq 1 ]]; then
  echo "ACT_DR6_R2: building pinned zero-safe AeST+memory CLASS..."
  bash nl1c6d2c6c_r5/setup_zero_safe_class.sh
  # shellcheck disable=SC1091
  source results/c3_r5_class_env.sh
else
  echo "ACT_DR6_R2: reusing CLASS at ${C3_R5_CLASS_ROOT}"
fi
...
LOG="results/act_dr6_lensing_exploratory_r2.log"
JSON="results/act_dr6_lensing_exploratory_r2.json"
NPZ="results/act_dr6_lensing_exploratory_r2.npz"
ZIP="results/act_dr6_lensing_exploratory_r2_bundle.zip"
rm -f "$LOG" "$JSON" "$NPZ" "$ZIP"

set +e
python -u obs_act_dr6_lensing/act_dr6_exploratory_scan_r2.py \
...
paths=[
    ...
    Path('obs_act_dr6_lensing/run_local_r2.sh'),
...
print(f'ACT_DR6_R2_BUNDLE={zip_path}')
PY

echo "ACT_DR6_R2_EXIT=$code"
echo "ACT_DR6_R2_LOG=$LOG"
echo "ACT_DR6_R2_JSON=$JSON"
echo "ACT_DR6_R2_NPZ=$NPZ"
echo "ACT_DR6_R2_ZIP=$ZIP"
exit "$code"
