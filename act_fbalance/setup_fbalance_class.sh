...
CLASS_ROOT="${CLASS_ROOT:-$ROOT/.local/class_fbalance_e8580832}"
PYTARGET="${PYTARGET:-$ROOT/.local/classy_fbalance_e8580832}"
...
import classy
print('FBALANCE_CLASSY_IMPORT_PASS')
print('classy_module=',classy.__file__)
c=Class(); c.empty()
PY

cat > "$ROOT/results/act_fbalance_class_env.sh" <<EOF
export FBALANCE_CLASS_ROOT="$CLASS_ROOT"
export FBALANCE_PYTARGET="$PYTARGET"
export PYTHONPATH="$PYTARGET\${PYTHONPATH:+:\$PYTHONPATH}"
EOF

printf 'FBALANCE_CLASS_ROOT=%s\nFBALANCE_PYTARGET=%s\n' "$CLASS_ROOT" "$PYTARGET"
printf 'FBALANCE_ENV=%s\n' "$ROOT/results/act_fbalance_class_env.sh"
