...
CLASS_ROOT="${CLASS_ROOT:-$ROOT/.local/class_c3_r4_e8580832}"
PYTARGET="${PYTARGET:-$ROOT/.local/classy_c3_r4_e8580832}"
...
grep -q 'eta0_tangent_force_aest' "$CLASS_ROOT/source/perturbations.c"
...
import classy
print("C3_R4_CORRECTED_CLASSY_IMPORT_PASS")
print("classy_module=", classy.__file__)
c=Class(); c.empty()
PY

cat > "$ROOT/results/c3_r4_class_env.sh" <<EOF
...
printf 'CLASS_ROOT=%s\nPYTARGET=%s\n' "$CLASS_ROOT" "$PYTARGET"
printf 'ENV_FILE=%s\n' "$ROOT/results/c3_r4_class_env.sh"
