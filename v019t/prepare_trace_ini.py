...
def rewrite(text,root):
    ...
    for line in text.splitlines():
        ...
        if s.startswith('root ='):
            ...
            if not seen_k:out.append(f'k_output_values = {KS}');seen_k=True
...
def main():
    ...
    for model,src in [('cosh','aest_cosh.ini'),('exp','aest_exp.ini')]:
        base=(repo/'v019'/'ini'/src).read_text();(dst/f'v019t_{model}.ini').write_text(rewrite(base,f'output/v019t_{model}_'))
        print(f'v019t_{model}.ini')
